...
*   **Status:** Partially DONE (Agent Integration Missing)
...
*   **Tasks:**
    ...
    *   **3.2:** Modify `ADK::Agent`. (TODO/UNKNOWN)
        *   Add mechanism to configure agent with MCP server connection details.
    ...
    *   **3.3:** Ensure `ADK::Planner` correctly sees and can select the dynamically registered MCP tools. (TODO/UNKNOWN - depends on 3.2)
*   **Testing:**
...
*   **Tasks:**
    ...
    *   **5.2:** Wrap `ADK::Tools::CheckJobStatusTool` using `AdkToolAdapter.wrap`. (TODO - Requires example/setup)
...
*   **Tasks:**
    *   **7.1:** Write comprehensive documentation for using the MCP client. (Partially DONE - Review `docs/mcp_integration.md`)
    *   **7.2:** Write comprehensive documentation for exposing tools/agents via `fast-mcp`. (Partially DONE - Review `docs/mcp_integration.md`, `docs/mcp_server_example_setup.md`)
    *   **7.3:** Create complete, runnable examples for both client and server scenarios. (Partially DONE - `docs/mcp_server_example_setup.md`, needs more)
    *   **7.4:** Refine error handling across all components. (TODO - Review needed)
    *   **7.5:** Review and enhance logging for clarity and debuggability. (TODO - Review needed)
    *   **7.6:** *(Optional)* Implement `ADK::Mcp::Connection::Sse`. (TODO)
    *   **7.7:** *(Optional)* Enhance schema conversion to handle more complex types. (TODO)
*   **Testing:** Good test coverage exists for implemented core components.
...
*   **Stateless Agent Adapter:** Implemented. `adk_direct_agent_adapter.rb` file exists alongside `adk_agent_adapter.rb`, needs clarification.
