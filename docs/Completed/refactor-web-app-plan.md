...
*   [X] Create file structure: `lib/adk/definition_store/`
*   [X] Create base module file: `lib/adk/definition_store.rb`
*   [X] Create Redis implementation file: `lib/adk/definition_store/redis_store.rb`
*   [X] Implement `ADK::DefinitionStore::RedisStore#initialize(redis_client)`:
    *   Takes a configured Redis client instance.
    *   Stores Redis client internally.
    *   Defines constants for Redis key prefixes/names.
*   [X] Implement `ADK::DefinitionStore::RedisStore#save_definition(name:, description:, tools:, model:, fallback_mode:, mcp_servers_json:)`:
    *   Takes agent definition data as arguments or a hash.
    *   Validates required fields (e.g., name).
    *   Uses Redis `MULTI` transaction.
*   [X] Implement `ADK::DefinitionStore::RedisStore#get_definition(agent_name)`:
    *   Takes agent name.
    *   Uses `HMGET` to retrieve all fields from `adk:agent:<name>`.
    *   Deserializes tools JSON and MCP JSON.
*   [X] Implement `ADK::DefinitionStore::RedisStore#update_definition(agent_name, updates_hash)`:
...
*   [X] Implement `ADK::DefinitionStore::RedisStore#delete_definition(agent_name)`:
    *   Takes agent name.
    *   Uses Redis `MULTI` transaction.
    *   Uses `DEL` to remove `adk:agent:<name>`.
*   [X] Implement `ADK::DefinitionStore::RedisStore#list_definitions()`:
...
*   [X] Implement `ADK::DefinitionStore::RedisStore#definition_exists?(agent_name)`:
    *   Takes agent name.
    *   Uses `SISMEMBER` on `adk:agents:all_names`.
    *   Returns `true` or `false`.
*   [X] Implement `ADK::DefinitionStore::RedisStore#check_connection()`:
...
*   [X] Modify `ADK::Web::App#initialize`:
...
*   [X] Refactor `GET /agents/:name/edit/:field`:
    *   Replace `@redis.exists?` check (or rely on store).
    *   Replace `@redis.hmget` with `@definition_store.get_definition(name)`.
    *   Adjust data access.
*   [X] Refactor `GET /agents/:name/display/*`:
    *   Replace `@redis.hmget` with `@definition_store.get_definition(name)`.
    *   Adjust data access.
    *   Check store availability.
*   [X] Refactor `PUT /agents/:name/update/:field`:
    *   Replace `@redis.exists?` check (or rely on store).
    ...
    *   Check store availability.
*   [X] Refactor `_start_agent` helper method:
    *   Replace `@redis.hmget` with `@definition_store.get_definition(name)`.
    *   Adjust data access.
    *   Check store availability.
*   [X] Refactor `GET /api/agents`:
    *   Replace `@redis.smembers` and pipelined `HMGET` with `@definition_store.list_definitions()`.
    *   Adjust data mapping.
    *   Check store availability.
*   [X] Refactor `GET /healthz`:
    *   Replace `@redis.ping` with `@definition_store.check_connection()`.
    *   Adjust error handling based on store method's return/exceptions.
    *   Check store availability.
*   [X] Refactor `GET /agents/:name/generate_example_task`:
    *   Replace `@redis.hmget` with `@definition_store.get_definition(name)`.
    *   Adjust data access.
    *   Handle `nil` return.
    *   Check store availability.
