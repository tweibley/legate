  ...
  module CLI
    ...
    class DeploymentCommands < Thor
      ...
      def run_gcloud_command(args, error_message)
        ...
        say "Executing: gcloud #{command_str}"

        # Safe execution using Open3 to prevent command injection
      ...
      def create_gcloud_config(base_name, project_id, region)
        ...
        # We can't easily check for command existence safely without shell unless we try to run it.
        # But 'command -v' is a shell builtin. The safest way is to assume it might fail if not present.
        # However, checking via system with fixed arguments is relatively safe.
        ...
        # We can use run_gcloud_command for this too, just ignoring the error message if it fails (not present)
        # But run_gcloud_command prints errors. Let's use Open3 directly for the check to keep it silent.
