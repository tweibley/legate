  ...
  module CLI
    ...
    class DeploymentCommands < Thor
      ...
      def run_gcloud_command(args, error_message)
        command_args = ['gcloud'] + args
        say "Executing: #{command_args.join(' ')}"

        output, status = Open3.capture2e(*command_args)
      ...
      def create_gcloud_config(base_name, project_id, region)
        ...
        # shell builtin 'command' requires shell, so we use string form safely with fixed arguments
