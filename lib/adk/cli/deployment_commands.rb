...
module ADK
  module CLI
    ...
    class DeploymentCommands < Thor
      ...
      # @param args [Array<String>|String] Command arguments (if string, it's unsafe but supported for legacy compatibility if strict splitting isn't possible)
      # @param error_message [String] Message to display on failure
      def run_gcloud_command(args, error_message)
        # Handle string input for backward compatibility, but warn or split if possible.
        # Ideally, we should receive an array.
        command_args = if args.is_a?(String)
                         # If it's a string, we really should have split it.
                         # Since we control the calls, we will update calls to pass arrays.
                         # But for safety, let's try to parse it with Shellwords if it's a string.
                         Shellwords.split(args)
                       else
                         args
                       end

        display_command = "gcloud #{command_args.join(' ')}"
        say "Executing: #{display_command}"

        output, status = Open3.capture2e('gcloud', *command_args)
      ...
      def create_gcloud_config(base_name, project_id, region)
        ...
        # Replace backticks with safe execution
        _, describe_status = Open3.capture2e('gcloud', 'config', 'configurations', 'describe', config_name)
