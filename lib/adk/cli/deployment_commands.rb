  ...
  module CLI
    ...
    class DeploymentCommands < Thor
      ...
      def run_gcloud_command(args, error_message)
        # args should be an array of strings to avoid shell injection
        ...
        begin
          ...
          say "Error: 'gcloud' command not found. Please install the Google Cloud SDK.", :red
          return false
        end
      ...
      def create_gcloud_config(base_name, project_id, region)
        ...
        # Use Open3 instead of system with shell redirection
        begin
          _, status = Open3.capture2e('gcloud', '--version')
          unless status.success?
            say "Error: 'gcloud' command not found (check failed). Cannot create gcloud configuration.", :red
        ...
        # Use run_gcloud_command (or Open3 direct) to check existence non-destructively
        # We suppress output here, so maybe just use Open3 directly
        _, check_status = Open3.capture2e('gcloud', 'config', 'configurations', 'describe', config_name)
