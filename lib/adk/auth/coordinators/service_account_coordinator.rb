  ...
  module Auth
    module Coordinators
      ...
      class ServiceAccountCoordinator < Coordinator
        ...
        # @param credential [ADK::Auth::Credential] The credential with service account information
        ...
        def generate_token_key
          ...
          if @credential[:client_email]
            base_key += ":#{@credential[:client_email]}"
          end
          
          # Add scopes if available
          if @scheme.scopes && !@scheme.scopes.empty?
            base_key += ":#{@scheme.scopes.join(',')}"
          end
          
          # Add audience if available and scopes aren't set
          if @scheme.audience && (@scheme.scopes.nil? || @scheme.scopes.empty?)
            base_key += ":#{@scheme.audience}"
          end
