  ...
  module Auth
    module Coordinators
      ...
      class OAuth2Coordinator < Coordinator
        ...
        # @param scheme [ADK::Auth::Schemes::OAuth2] The OAuth2 scheme
        ...
        def initialize(scheme:, credential:, session_service:, token_store: nil, timeout: DEFAULT_TIMEOUT, redirect_uri: nil)
          ...
          unless scheme.is_a?(ADK::Auth::Schemes::OAuth2)
            raise ArgumentError, "Expected an OAuth2 scheme, got #{scheme.class}"
          end
          
          unless credential.auth_type == :oauth2
            raise ArgumentError, "Credential must have auth_type :oauth2, got #{credential.auth_type}"
          end
        ...
        def request_authorization
          ...
          unless response.is_a?(Hash)
            raise ADK::Auth::Error, "Invalid authorization response: expected Hash, got #{response.class}"
          end
          
          unless response['response_uri']
            raise ADK::Auth::Error, "Missing response_uri in authorization response"
          end
