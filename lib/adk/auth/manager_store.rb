# File: lib/adk/auth/manager_store.rb
...
module ADK
  module Auth
    ...
    module ManagerStore
      ...
      class RedisStore
        # Redis key prefixes
        SCHEMES_HASH_KEY = 'adk:auth:schemes'
        CREDENTIALS_HASH_KEY = 'adk:auth:credentials'
        URL_MAPPINGS_KEY = 'adk:auth:url_mappings'

        # @param redis_client [Redis] An instance of the Redis client
        def initialize(redis_client:)
          @redis = redis_client
          @logger = ADK.logger
          @logger&.info('ADK::Auth::ManagerStore::RedisStore initialized.')
        rescue => e
          ADK.logger&.error("Failed to initialize Auth ManagerStore: #{e.message}")
          @redis = nil
        end
        ...
        # @param scheme [ADK::Auth::Scheme] The scheme to save
        ...
        # @param credential [ADK::Auth::Credential] The credential to save
        # @return [Boolean] true if successful
        ...
        def load_url_mappings
          ...
          mappings.map do |mapping|
            ...
            if pattern.is_a?(Hash) && pattern[:regexp]
              pattern = Regexp.new(pattern[:regexp])
            end
        ...
        def serialize_scheme(scheme)
          ...
          case scheme
          when ADK::Auth::Schemes::OAuth2, ADK::Auth::Schemes::OpenIDConnect
          ...
          when ADK::Auth::Schemes::ServiceAccount
            data[:token_url] = scheme.token_url if scheme.respond_to?(:token_url)
          when ADK::Auth::Schemes::GoogleServiceAccount
            data[:scopes] = scheme.scopes if scheme.respond_to?(:scopes)
          end
      ...
      class InMemoryStore
        def initialize
          ...
          @url_mappings = []
          @logger = ADK.logger
          @logger&.info('ADK::Auth::ManagerStore::InMemoryStore initialized (no persistence).')
        end
