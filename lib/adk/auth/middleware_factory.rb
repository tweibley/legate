# File: lib/adk/auth/middleware_factory.rb
  ...
  module Auth
    ...
    class MiddlewareFactory
      class << self
        ...
        # @option options [ADK::Auth::TokenStore] :token_store Optional token store for caching tokens
        # @option options [ADK::Auth::TokenManager] :token_manager Optional token manager for token lifecycle
        ...
        def create(scheme:, credential:, **options)
          ...
          unless token_store
            ...
            token_store = ADK::Auth::TokenStore.new(session_service) if session_service
          end
        ...
        def create_oauth2(client_id:, client_secret:, authorization_url:, token_url:, scopes: nil, **options)
          ...
          # Create the scheme
          scheme = ADK::Auth::Schemes::OAuth2.new(**scheme_options)
        ...
        def create_oidc(client_id:, client_secret:, discovery_url: nil, authorization_url: nil, 
          ...
          # Create the scheme
          scheme = ADK::Auth::Schemes::OIDC.new(**scheme_options)
        ...
        def create_from_provider(provider_id, **options)
          # Retrieve the stored credential from ADK::Auth
          exchanged_credential = ADK::Auth.get_exchanged_credential(provider_id)
          unless exchanged_credential
            raise ArgumentError, "No credential found for provider ID: #{provider_id}"
          end
          
          # Get the scheme
          scheme = ADK::Auth.get_scheme_for_provider(provider_id)
          unless scheme
            raise ArgumentError, "No scheme found for provider ID: #{provider_id}"
          end
