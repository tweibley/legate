...
module ADK
  module Auth
    ...
    class Coordinator
      ...
      # @param session_service [ADK::SessionService::Base] The session service for state persistence
      ...
      def start
        ...
        @auth_fiber = Fiber.new do
          begin
            ...
            @error = ADK::Auth::Error.new("Unexpected error during authentication: #{e.message}", e)
            @status = Status::FAILED
            nil
          end
        end
      ...
      # @return [ADK::Auth::ExchangedCredential, nil] The resulting credential or nil on failure
      def resume(response)
        raise ADK::Auth::Error, "Authentication flow is not in progress (status: #{@status})" unless @status == Status::PENDING
        
        # Check for timeout
        if @timeout && Time.now - @start_time > @timeout
          @status = Status::TIMEOUT
          @error = ADK::Auth::Error.new("Authentication timed out after #{@timeout} seconds")
          save_auth_state
          return nil
        end
        ...
        begin
          ...
          if @result && @result.is_a?(ADK::Auth::ExchangedCredential) && @token_store
          ...
          @error = ADK::Auth::Error.new("Error resuming authentication: #{e.message}", e)
          @status = Status::FAILED
          save_auth_state
          nil
        end
      ...
      def cancel(reason = nil)
        ...
        @error = ADK::Auth::Error.new("Authentication cancelled#{reason ? ": #{reason}" : ''}")
        save_auth_state
        true
      end
      ...
      # @return [ADK::Auth::Error, nil] The error or nil if no error occurred
      def error
        @error
      end

      # Get the result of the authentication flow
      # @return [ADK::Auth::ExchangedCredential, nil] The resulting credential or nil if not completed
      def result
        @result
      end
