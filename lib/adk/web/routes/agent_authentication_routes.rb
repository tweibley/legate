# File: lib/adk/web/routes/agent_authentication_routes.rb
    ...
    module AgentAuthenticationRoutes
      def self.registered(app)
          ...
          def get_agent_auth_status(agent_name)
            ...
            begin
              ...
              scheme_assignments.each do |service, scheme_name|
                unless auth_manager.get_scheme(scheme_name.to_sym)
                  issues << "Scheme '#{scheme_name}' not found for service '#{service}'"
                end
              end

              credential_assignments.each do |service, credential_name|
                unless auth_manager.get_credential(credential_name.to_sym)
                  issues << "Credential '#{credential_name}' not found for service '#{service}'"
                end
              end
          ...
          def test_agent_authentication(agent_name, test_options = {})
            ...
            begin
              ...
              if test_options[:test_url]
                ...
                if resolved_auth[:scheme] && resolved_auth[:credential]
                  test_results[:tests] << {
        ...
        app.get '/agents/:name/auth' do |name|
          ...
          self.instance_variable_set(:@agent_auth_data, agent_auth_data)
          slim :agent_auth
        rescue => e
        ...
        app.post '/agents/:name/auth/url-mapping' do |name|
          ...
          begin
            ...
            unless compatible_types.include?(credential.auth_type.to_s) || compatible_types.include?(credential.auth_type)
