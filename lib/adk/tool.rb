...
module ADK
  ...
  class Tool
    ...
    def validate_and_coerce_params(params)
      ...
      # Note: transform_keys returns a NEW hash, so we can modify it safely without duping again.
      coerced_params = params.transform_keys(&:to_sym)

      current_parameters = @parameters || {}

      # 2. Check for missing required parameters
      # Use pre-calculated symbol keys if available
      required_param_names = @required_parameters || current_parameters.select { |_, p| p[:required] }.keys
