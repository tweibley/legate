...
module ADK
  ...
  class Tool
    ...
    def initialize(**_options)
      ...
      # Optimize validation by pre-calculating required parameters
      # This avoids iterating over the hash on every execution
    ...
    def validate_and_coerce_params(params)
      ...
      # Use pre-calculated required params if available (optimization), otherwise calculate
      req_param_names = defined?(@required_param_names) ? @required_param_names : current_parameters.select { |_, p| p[:required] }.keys

      present_keys = normalized_params.keys
      missing_params = req_param_names - present_keys
