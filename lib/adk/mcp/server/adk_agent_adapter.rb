    ...
    module Server
      ...
      class AdkAgentAdapter < FastMcp::Tool
        ...
        def self.wrap(agent_definition_name, session_service_instance)
          ...
          raise ArgumentError, 'Session service instance must inherit from ADK::SessionService::Base.' unless session_service_instance.is_a?(ADK::SessionService::Base)

          # Create the anonymous adapter class
          Class.new(AdkAgentAdapter) do
        ...
        def call(prompt:)
          ...
          unless agent_name && session_service
            raise NotImplementedError,
                  'AdkAgentAdapter must be configured using .wrap first.'
          end

          Mcp.logger.info("Executing ADK Agent '#{agent_name}' via MCP adapter with prompt: '#{prompt}'")

          begin
            agent_definition_object = load_agent_definition(agent_name)

            with_temporary_session(agent_name, session_service) do |temp_session, agent_cleanup_block|
              Mcp.logger.debug("Instantiating agent '#{agent_definition_object.name}'...")
              agent = ADK::Agent.new(
                definition: agent_definition_object,
                session_service: session_service
              )
              # Register agent stop for cleanup
              agent_cleanup_block.call(agent)
              ...
              process_result(final_event, agent_name)
            end
        ...
        def load_agent_definition(agent_name)
          ...
          raise ADK::Mcp::Error, "Agent definition '#{agent_name}' not found in Redis or is empty." if definition_hash_from_redis.empty?

          definition_hash = definition_hash_from_redis.transform_keys(&:to_sym)
          if definition_hash[:tools].is_a?(String)
            definition_hash[:tools] = begin
              JSON.parse(definition_hash[:tools])
            rescue StandardError
              []
            end
          end
          ...
          Mcp.logger.debug("Creating AgentDefinition object for '#{agent_name}'.")
          agent_definition = ADK::AgentDefinition.from_hash(definition_hash)

          unless agent_definition
            Mcp.logger.error("Failed to create AgentDefinition for '#{agent_name}'. Hash: #{definition_hash.inspect}")
            raise ADK::Mcp::Error, "Could not create a valid AgentDefinition object for '#{agent_name}'."
          end

          Mcp.logger.debug("AgentDefinition created: #{agent_definition.name}, Model=#{agent_definition.model_name}")
          agent_definition
        end

        def with_temporary_session(agent_name, session_service)
          ...
          agent_ref = nil
          # Block to allow caller to register agent for cleanup
          agent_cleanup_callback = ->(agent) { agent_ref = agent }

          begin
            yield(temp_session, agent_cleanup_callback)
          ensure
            cleanup_resources(agent_ref, temp_session, session_service)
          end
        end

        def cleanup_resources(agent, session, session_service)
          if agent&.running?
            begin
          ...
          return unless session && session_service

          begin
            Mcp.logger.debug("Deleting temporary session: #{session.id}")
            session_service.delete_session(session_id: session.id)
          rescue StandardError => e
            Mcp.logger.error("Error deleting temporary session #{session.id}: #{e.message}")
          end
        end

        def process_result(final_event, agent_name)
          raise StandardError, "Agent task finished with unexpected event format: #{final_event.inspect}" unless final_event.is_a?(ADK::Event) && final_event.role == :agent && final_event.content.is_a?(Hash)
          ...
          when :pending
            ...
            Mcp.logger.warn("Agent '#{agent_name}' pending (Job: #{job_id}). Returning as structured data.")
