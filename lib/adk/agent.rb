...
module ADK
  ...
  class Agent
    ...
    def initialize(definition:, session_service: nil, planner_override: nil, sub_agents: nil)
      ...
      @mcp_connection_manager = ADK::Mcp::ConnectionManager.new(ADK.logger)
    ...
    def connect_mcp_servers
      ...
      @mcp_connection_manager.connect_all(@mcp_servers_config)
      @mcp_connection_manager.register_tools(@tool_registry, @selected_tool_names)
    end
    ...
    # @deprecated Use ADK::Mcp::ConnectionManager#register_tools instead.
    def discover_and_register_mcp_tools(client)
      # This method is kept for backward compatibility if called directly,
      # but it's now better to rely on ConnectionManager.
      ADK.logger.warn('ADK::Agent#discover_and_register_mcp_tools is deprecated. Use ADK::Mcp::ConnectionManager instead.')

      # We create a temporary list of 1 client to reuse the manager's logic, or just reimplement basics
      # Since we have the client, let's just use the logic from the manager manually here
      begin
        ...
        ADK.logger.error("Error in deprecated discover_and_register_mcp_tools: #{e.message}")
      end
    end
