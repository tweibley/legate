...
module ADK
  ...
  #
  # This class acts as a bridge between a tool and the broader agent environment.
  # It grants tools access to session-specific data, the user's identity, and a
  # mechanism to share state across different tool executions within the same session.
  #
  ...
  class ToolContext
    ...
    # @return [Hash] The accumulated state changes to be applied after successful tool execution
    ...
    # Retrieves a value from the persistent session state.
    #
    # This state allows tools to share information across different steps in a session.
    #
    # @param key [Symbol, String] The key to retrieve
    # @return [Object, nil] The value associated with the key, or nil if not found
    #
    # @example Retrieve a user preference
    #   theme = context.state_get(:theme)
    #
    def state_get(key)
    ...
    # Stages a value to be saved to the session state.
    #
    # Changes made via `state_set` are "pending" and are only committed to the persistent
    # session state after the tool execution successfully completes. This ensures atomic updates.
    #
    # @param key [Symbol, String] The key to set
    # @param value [Object] The value to store (must be serializable, e.g., String, Integer, Hash)
    #
    # @example Increment a counter
    #   context.state_set(:counter, 1)
    #
    ...
    # Stages multiple values to be saved to the session state.
    #
    # Like `state_set`, these changes are pending until successful tool execution.
    #
    # @param hash_to_merge [Hash] A hash of keys and values to merge into the state
    #
    # @example Update multiple user details
    #   context.state_update(name: "Alice", role: "admin")
    #
