...
module ADK
  ...
  Event = Struct.new(:role, :content, :timestamp, :tool_name, :event_id, keyword_init: true) do
    ...
    def initialize(role:, content:, timestamp: nil, tool_name: nil, event_id: nil)
      # Basic validation
    ...
    def self.from_h(hash)
      new(
        role: hash[:role]&.to_sym, # Safely convert role to symbol
        content: hash[:content],
        # Safely parse timestamp
        timestamp: hash[:timestamp] ? Time.iso8601(hash[:timestamp]) : Time.now.utc,
        tool_name: hash[:tool_name]&.to_sym, # Safely convert tool_name to symbol
