...
RSpec.describe Legate::AgentDefinition do
  ...
  describe 'DSL methods (via #define)' do
    ...
    it 'defaults planning_strategy to :plan' do
      definition.define { |a| minimal_valid_block_proc.call(a) }
      expect(definition.planning_strategy).to eq(:plan)
    end

    it 'allows setting planning_strategy to :react' do
      definition.define do |a|
        minimal_valid_block_proc.call(a)
        a.planning_strategy :react
      end
      expect(definition.planning_strategy).to eq(:react)
    end

    it 'raises ArgumentError for invalid planning_strategy values' do
      expect do
        definition.define do |a|
          minimal_valid_block_proc.call(a)
          a.planning_strategy :bogus
        end
      end.to raise_error(ArgumentError, /Planning strategy must be one of:/)
    end

    it 'round-trips planning_strategy through to_h / from_hash' do
      definition.define do |a|
        minimal_valid_block_proc.call(a)
        a.planning_strategy :react
      end
      restored = Legate::AgentDefinition.from_hash(definition.to_h)
      expect(restored.planning_strategy).to eq(:react)
    end
  ...
  describe '#validate!' do
    ...
    context 'when webhook_enabled is true' do
      ...
      it 'logs a warning if webhook_transformer is not a Proc' do
        expect(Legate.logger).to receive(:warn) { |&block|
  ...
  describe '#to_h' do
    ...
    it 'shows nil for webhook_secret if not set' do
      local_definition = Legate::AgentDefinition.new
  ...
  describe '.from_hash and #to_h serialization/deserialization' do
    ...
    context 'when all attributes are set' do
      ...
      it 'reconstructs the tool_names correctly' do
        expect(reconstructed_definition.tool_names).to match_array(%i[tool_one tool_two])
      end
      ...
      it 'reconstructs webhook_secret as the string <present> (due to to_h behavior)' do
        ...
        # Legate::AgentDefinition.from_hash receives '<present>' and sets it.
      ...
      it 'reconstructs webhook_transformer as nil (Procs are not serialized by to_h)' do
        ...
        # Legate::AgentDefinition.from_hash cannot reconstruct the Proc from this string.
      ...
      it 'reconstructs sub_agent_names correctly' do
        expect(reconstructed_definition.sub_agent_names).to match_array(%i[sub_agent_alpha sub_agent_beta])
      end
      ...
      it 'reconstructs sequential_sub_agent_names correctly' do
        expect(reconstructed_definition.sequential_sub_agent_names).to match_array(%i[seq_agent_1 seq_agent_2])
      end

      it 'reconstructs parallel_sub_agent_names correctly' do
        expect(reconstructed_definition.parallel_sub_agent_names).to match_array(%i[par_agent_1 par_agent_2])
      end
      ...
      it 'reconstructs delegation_targets correctly' do
        expect(reconstructed_definition.delegation_targets).to match_array(%i[target_agent_1 target_agent_2])
      end
  ...
  describe '#use_tool' do
    let(:weather_tool_class) do
      Class.new(Legate::Tool) do
        self.explicit_tool_name = :weather
        tool_description 'Reports the weather'
        def perform_execution(_params, _ctx) = { status: :success, result: 'sunny' }
      end
    end

    def define_with(&block)
      described_class.new.define do |a|
        a.name :tooly
        a.instruction 'x'
        block.call(a)
      end
    ...
    it 'accepts a Symbol (existing behavior)' do
      definition = define_with { |a| a.use_tool :echo }
      expect(definition.tool_names).to include(:echo)
    end

    it 'coerces a String tool name to a Symbol' do
      definition = define_with { |a| a.use_tool 'echo' }
      expect(definition.tool_names.to_a).to eq([:echo])
      expect(definition.tool_names.first).to be_a(Symbol)
    end

    it 'registers a Tool subclass globally and selects it by its metadata name' do
      klass = weather_tool_class
      definition = define_with { |a| a.use_tool klass }
      expect(definition.tool_names).to include(:weather)
      expect(Legate::GlobalToolManager.find_class(:weather)).to eq(klass)
    end

    it 'raises for an unsupported argument type' do
      expect { define_with { |a| a.use_tool 123 } }
        .to raise_error(ArgumentError, %r{Symbol/String.*or a Legate::Tool subclass})
    end
  end
end
