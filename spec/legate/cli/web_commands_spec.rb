...
require 'spec_helper'
require 'legate/cli/web_commands'
require 'rack'
require 'rack/handler/puma'

RSpec.describe Legate::CLI::WebCommands do
  ...
  before do
    ...
    # Mock Rack::Handler::Puma to prevent actual server startup
  ...
  describe '#start' do
    context 'with default options' do
      it 'starts Rack server with default host and port' do
        expect(Rack::Handler::Puma).to receive(:run).with(anything, hash_including(Host: 'localhost', Port: 4567))
        invoke_command(:start)
        expect(output.string).to include('Starting Legate web interface on http://localhost:4567')
      end
    ...
    context 'with custom host and port' do
      it 'starts Rack server with specified host and port' do
        expect(Rack::Handler::Puma).to receive(:run).with(anything, hash_including(Host: '0.0.0.0', Port: 8080))
    ...
    context 'Webhook Listener integration' do
      it 'mounts webhook listener if enabled' do
        # Mock Legate.config
        allow(Legate.config.webhooks).to receive(:listener_enabled).and_return(true)
        allow(Legate.config.webhooks).to receive(:base_path).and_return('/webhooks')
        ...
        # Typically Legate.logger goes to STDOUT, but tests capture it or suppress it.
