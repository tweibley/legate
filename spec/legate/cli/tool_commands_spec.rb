...
require 'spec_helper'
require 'legate/cli/tool_commands'
...
class MockCliTestTool < Legate::Tool
  ...
  def perform_execution(params, _context)
    if params[:param1] == 'error'
      raise Legate::ToolError, 'Simulated tool error'
...
RSpec.describe Legate::CLI::ToolCommands do
  ...
  before do
    ...
    # Mock Legate::GlobalToolManager methods (the actual class used by ToolCommands)
    allow(Legate::GlobalToolManager).to receive(:list_all_tools).and_return([])
    allow(Legate::GlobalToolManager).to receive(:create_instance).and_return(nil)
  ...
  describe '#list' do
    ...
    context 'when tools are registered' do
      ...
      before do
        allow(Legate::GlobalToolManager).to receive(:list_all_tools).and_return(tool_list)
      end
  ...
  describe '#info' do
    ...
    context 'when tool exists' do
      ...
      before do
        allow(Legate::GlobalToolManager).to receive(:create_instance).with(:mock_tool).and_return(tool_instance)
  ...
  describe '#execute' do
    ...
    context 'when tool does not exist' do
      before do
        allow(Legate::GlobalToolManager).to receive(:create_instance).with(:unknown).and_return(nil)
      end
    ...
    context 'when tool exists' do
      ...
      it 'handles single argument as required parameter if applicable' do
        ...
        allow(Legate::GlobalToolManager).to receive(:create_instance).with(:single).and_return(single_param_tool)
