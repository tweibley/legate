...
require 'spec_helper'
require 'legate/activity_log'
require 'timecop'

RSpec.describe Legate::ActivityLog do
  ...
  describe '#log' do
    ...
    it 'limits the number of events stored' do
      max_events = Legate::ActivityLog::MAX_EVENTS
    ...
    it 'is thread-safe' do
      ...
      expect(activity_log.recent(100).size).to eq([100, Legate::ActivityLog::MAX_EVENTS].min)
    end
  end
  ...
  describe 'Class methods delegation' do
    ...
    it 'safe_log records an event like log' do
      described_class.clear
      described_class.safe_log(:safe_event, { name: 'x' })
      expect(described_class.recent(1).first).to include(type: :safe_event)
    end

    it 'safe_log swallows errors so logging never breaks the caller' do
      allow(described_class).to receive(:log).and_raise(StandardError, 'boom')
      allow(Legate.logger).to receive(:debug)
      expect { described_class.safe_log(:explodes) }.not_to raise_error
    end
