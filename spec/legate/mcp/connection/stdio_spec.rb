# File: spec/legate/mcp/connection/stdio_spec.rb
# frozen_string_literal: true

require 'spec_helper'
require 'legate/mcp/connection/stdio'
require 'legate/errors'
require 'legate/mcp' # For logger
require 'stringio'
require 'timeout'

RSpec.describe Legate::Mcp::Connection::Stdio do
  ...
  before do
    allow(Legate::Mcp).to receive(:logger).and_return(logger_spy)
  ...
  describe '#connect (integration-like)' do
    ...
    it 'raises ConnectionError if command not found' do
      ...
      expect { connection.connect }.to raise_error(Legate::Mcp::ConnectionError, /Command not found/)
    end
  end
  ...
  describe '#send_request' do
    ...
    it 'raises ConnectionError if disconnected (state check)' do
      ...
      expect { connection.send_request(request_hash) }.to raise_error(Legate::Mcp::ConnectionError, 'Not connected')
    end
    ...
    it 'raises ConnectionError on EPIPE' do
      ...
      expect {
        connection.send_request(request_hash)
      }.to raise_error(Legate::Mcp::ConnectionError, /MCP process stdin pipe broke/)
      expect(connection.connected?).to be false
    end
  end
  ...
  describe '#read_message' do
    ...
    it 'raises ConnectionError if disconnected (state check)' do
      connection.instance_variable_set(:@connected, false)
      expect { connection.read_message }.to raise_error(Legate::Mcp::ConnectionError)
    end
    ...
    context 'when waiting with timeout' do
      ...
      it 'returns nil if timeout occurs before message arrives' do
        result = connection.read_message(0.1)
      ...
      it 'raises ConnectionError if connection drops during wait' do
        ...
        }.to raise_error(Legate::Mcp::ConnectionError, 'Connection lost while waiting for message')
      end
    end
  ...
  describe '#disconnect' do
    ...
    it 'joins and cleans up reader threads' do
      ...
      allow(stderr_thread_spy).to receive(:alive?).and_return(true)
      allow(stdout_thread_spy).to receive(:alive?).and_return(true)
      expect(stderr_thread_spy).to receive(:join).with(2).and_return(nil)
      expect(stderr_thread_spy).to receive(:kill)
      expect(stdout_thread_spy).to receive(:join).with(2).and_return(nil)
    ...
    it 'sends SIGKILL if process does not exit after timeout and force: true' do
      ...
      expect(mock_wait_thr).to receive(:join).with(2)
  ...
  describe 'Reader Thread Logic (Direct Testing)' do
    ...
    def run_stdout_logic(io, initial_connected_state, response_q, notify_q)
      ...
      last_err = nil
      threshold = Legate::Mcp::Connection::Stdio::PARSE_ERROR_THRESHOLD
    ...
    context 'stdout processing' do
      ...
      it 'stops processing and sets state after PARSE_ERROR_THRESHOLD errors' do
        stub_const('Legate::Mcp::Connection::Stdio::PARSE_ERROR_THRESHOLD', 2)
      ...
      it 'resets parse error count on valid JSON' do
        stub_const('Legate::Mcp::Connection::Stdio::PARSE_ERROR_THRESHOLD', 3)
