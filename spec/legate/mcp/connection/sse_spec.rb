# File: spec/legate/mcp/connection/sse_spec.rb
# frozen_string_literal: true

require 'spec_helper'
require 'net/http'
require 'legate/mcp/connection/sse'
require 'legate/errors'
require 'legate' # For logger
require 'stringio' # For mocking streams

RSpec.describe Legate::Mcp::Connection::Sse do
  ...
  before do
    ...
    # Stub Net::HTTP.start with correct host/port/timeouts for SSE connection
    allow(Net::HTTP).to receive(:start).with(sse_uri.hostname, sse_uri.port,
                                             use_ssl: false, open_timeout: 5,
                                             read_timeout: 30).and_yield(mock_http)
  ...
  describe '#connect' do
    ...
    before do
      ...
      allow(sse_ok_response).to receive(:read_body) { sleep }
      allow(sse_ok_response).to receive(:is_a?).with(Net::HTTPOK).and_return(true)

      allow(connection).to receive(:process_sse_event)
    end
    ...
    it 'raises ConnectionError if SSE connection fails (non-200)' do
      bad_response = instance_double('SSE Response', :[] => 'text/html', code: '404')
      ...
      expect {
        connection.connect
      }.to raise_error(Legate::Mcp::ConnectionError, /SSE endpoint returned 404/)
      expect(connection.connected?).to be false
    end

    it 'raises ConnectionError if SSE connection has wrong content type' do
      wrong_type_response = instance_double('SSE Response', :[] => 'application/json', code: '200')
      ...
      expect { connection.connect }.to raise_error(Legate::Mcp::ConnectionError, /SSE endpoint returned 200/)
      expect(connection.connected?).to be false
    end
    ...
    it 'raises ConnectionError on network errors' do
      ...
      expect { connection.connect }.to raise_error(Legate::Mcp::ConnectionError, /Connection refused/)
      expect(connection.connected?).to be false
    end
  ...
  describe '#disconnect' do
    ...
    it 'kills the reader thread and resets state' do
      ...
      expect(thread_spy).to receive(:join).with(2)
  ...
  describe '#send_request' do
    ...
    let(:mcp_error_response_body) {
      { jsonrpc: '2.0', id: mcp_request_hash[:id], error: { code: -32_000, message: 'Server error' } }.to_json
    }
    ...
    it 'raises ConnectionError for non-200 HTTP responses' do
      ...
      }.to raise_error(Legate::Mcp::ConnectionError, /MCP POST request failed: 500/)
    end
    ...
    it 'raises RemoteToolError if non-200 response contains MCP error structure' do
      ...
      expect { connection.send_request(mcp_request_hash) }.to raise_error(Legate::Mcp::RemoteToolError, /Server error/)
    end
    ...
    it 'raises ProtocolError if the successful response body is not valid JSON' do
      ...
      expect {
        connection.send_request(mcp_request_hash)
      }.to raise_error(Legate::Mcp::ProtocolError,
    ...
    it 'raises ConnectionError on network errors' do
      ...
      expect {
        connection.send_request(mcp_request_hash)
      }.to raise_error(Legate::Mcp::ConnectionError, /Connection refused/)
    end
  end
