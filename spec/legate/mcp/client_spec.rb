# File: spec/legate/mcp/client_spec.rb
# frozen_string_literal: true

require 'spec_helper'
require 'legate/version'
require 'legate/mcp/client'
require 'legate/mcp/connection/stdio' # Need the class for mocking
require 'legate/mcp/connection/sse'   # Add for new tests
require 'legate/errors'
require 'legate/mcp' # For logger

# Define constants needed for tests
Legate::VERSION = '0.1.0' unless defined?(Legate::VERSION)
# Need to access the client's constant
PROTOCOL_VERSION = Legate::Mcp::Client::CLIENT_PROTOCOL_VERSION

RSpec.describe Legate::Mcp::Client do
  ...
  let(:mock_connection) do
    instance_double(Legate::Mcp::Connection::Stdio,
                    connect: true,
                    disconnect: true,
                    connected?: true,
                    send_request: nil,
  ...
  before do
    ...
    allow(Legate::Mcp::Connection::Stdio).to receive(:new)
  ...
  describe '#connect' do
    ...
    before do
      ...
      allow(client).to receive(:send_request_and_wait)
        .with(initialize_request, timeout: Legate::Mcp::Connection::Stdio::PROCESS_START_TIMEOUT)
        .and_return(initialize_response_success)
    end

    it 'creates and connects the connection object' do
      expect(Legate::Mcp::Connection::Stdio).to receive(:new).and_return(mock_connection)
    ...
    it 'raises ConnectionError and disconnects if connection.connect fails' do
      allow(mock_connection).to receive(:connect).and_raise(Legate::Mcp::ConnectionError, 'Process failed')
      expect(client).to receive(:disconnect)
      expect { client.connect }.to raise_error(Legate::Mcp::ConnectionError, 'Process failed')
    end
    ...
    it 'raises ConnectionError and disconnects if handshake response is missing' do
      ...
      expect(client).to receive(:disconnect)
      expect { client.connect }.to raise_error(Legate::Mcp::ConnectionError, /MCP Initialize failed: No response/)
    end
    ...
    it 'raises ConnectionError and disconnects if handshake response has no result' do
      ...
      expect {
        client.connect
      }.to raise_error(Legate::Mcp::ConnectionError, /MCP Initialize failed: No response or missing result/)
    end
    ...
    it 'raises ConnectionError on unexpected errors during connect' do
      ...
      expect(client).to receive(:disconnect)
      expect { client.connect }.to raise_error(Legate::Mcp::ConnectionError, /Unexpected boom/)
    end
  ...
  describe '#list_tools' do
    ...
    it 'raises ConnectionError if not connected' do
      ...
      expect { client.list_tools }.to raise_error(Legate::Mcp::ConnectionError, 'Not connected')
    end
    ...
    it 'raises ProtocolError if result.tools is not an array' do
      ...
      expect { client.list_tools }.to raise_error(Legate::Mcp::ProtocolError, /is not an Array/)
    end

    it 'raises RemoteToolError if server returns an error' do
      error_response = { jsonrpc: '2.0', id: 2, error: { code: -32_001, message: 'Server error', data: 'details' } }
      expect(client).to receive(:send_request_and_wait).with(list_request).and_return(error_response)
      expect { client.list_tools }.to raise_error(Legate::Mcp::RemoteToolError, %r{MCP tools/list failed: Server error})
    end
    ...
    it 'raises ProtocolError if response is invalid or missing' do
      ...
      expect { client.list_tools }.to raise_error(Legate::Mcp::ProtocolError, /Invalid or missing response/)
  ...
  describe '#call_tool' do
    ...
    it 'raises ConnectionError if not connected' do
      allow(client).to receive(:connected?).and_return(false)
      expect { client.call_tool(tool_name, tool_args) }.to raise_error(Legate::Mcp::ConnectionError, 'Not connected')
    end
    ...
    it 'raises RemoteToolError if server returns an error' do
      error_response = { jsonrpc: '2.0', id: 3,
                         error: { code: -32_002, message: 'Invalid params', data: { field: 'a' } } }
    ...
    it 'raises ProtocolError if response is invalid or missing' do
      ...
      }.to raise_error(Legate::Mcp::ProtocolError, /Invalid or missing response/)
