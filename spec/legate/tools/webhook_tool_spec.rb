# File: spec/legate/tools/webhook_tool_spec.rb
...
Legate.logger.level = Logger::FATAL # Quieten logs for tests
...
class MockToolContext
  ...
  def initialize(session: nil, tool_registry: nil)
    @session = session || instance_double('Legate::Session')
    @tool_registry = tool_registry || instance_double('Legate::GlobalToolManager')
  end
...
RSpec.describe Legate::Tools::WebhookTool do
  ...
  describe '#execute' do
    before do
      # Stub SSRF validation to avoid real DNS and IP pinning in HTTP behavior tests
      allow(tool).to receive(:validate_url_security).and_return(nil)
    end
    ...
    context 'with basic hash payload' do
      ...
      it 'sends POST with JSON payload and default headers' do
        ...
        expect(a_request(:post, url).with { |req|
          req.headers['User-Agent'].start_with?('Legate-Ruby/')
        }).to have_been_made
      end
    end
    ...
    context 'with invalid URL' do
      ...
      it 'raises ToolArgumentError' do
        ...
        expect {
          tool.execute(params,
                       context: context)
        }.to raise_error(Legate::ToolArgumentError, /Invalid URL provided: invalid url - bad URI/)
      end
    end
    ...
    context 'when http_post fails (e.g., timeout)' do
      ...
      it 'raises the underlying ToolError' do
        ...
        }.to raise_error(Legate::ToolTimeoutError, /Timeout during POST request/)
      end
    end
  end
end
