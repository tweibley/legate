...
# File: spec/legate/tools/echo_spec.rb
require 'spec_helper' # Loads 'legate'

RSpec.describe Legate::Tools::Echo do
  ...
  describe '#execute' do
    ...
    context 'with missing required parameters' do
      ...
      # Updated to expect Legate::ToolArgumentError
      it 'raises an Legate::ToolArgumentError' do
        expect { tool.execute(params) }.to raise_error(Legate::ToolArgumentError, /Missing required parameters for tool 'echo': message/)
      end
    end
    ...
    context 'when message is unexpectedly nil in perform_execution (post-validation)' do
      ...
      it 'logs an error and raises a ToolError' do
        ...
        allow(Legate.logger).to receive(:error) # Prevent logging noise

        expect { tool.execute(params) }
          .to raise_error(Legate::ToolError, /Internal Error: Message parameter missing/)

        expect(Legate.logger).to have_received(:error).with(/Internal Error: Message parameter missing/).at_least(:once)
      end
    end
    ...
    context 'when an unexpected error occurs during fetch' do
      ...
      it 'logs the error and raises a ToolError' do
        ...
        expect { tool.execute(params) }
          .to raise_error(Legate::ToolError, /Unexpected error in Echo tool: Something broke!/)

        expect(Legate.logger).to have_received(:error).with('Echo Tool: Unexpected error: StandardError - Something broke!')
      end
    end
