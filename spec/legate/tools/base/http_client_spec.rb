# File: spec/legate/tools/base/http_client_spec.rb
...
Legate.logger.level = Logger::FATAL # Set directly for tests
...
RSpec.describe Legate::Tools::Base::HttpClient do
  ...
  around do |example|
    ...
    original_log_device = Legate.logger.instance_variable_get(:@logdev)&.dev
    original_level = Legate.logger.level
    logdev = Legate.logger.instance_variable_get(:@logdev)
    logdev&.instance_variable_set(:@dev, logger_output)
    Legate.logger.level = Logger::DEBUG # Ensure logs are captured
    ...
    Legate.logger.level = original_level
  ...
  describe '#setup_http_client' do
    context 'with valid base URL' do
      ...
      it 'sets default headers including User-Agent' do
        expected_ua = "Legate-Ruby/#{Legate::VERSION} #{Excon::USER_AGENT}"
      ...
      it 'configures logging instrumentor' do
        expect(tool_instance.http_client.data[:instrumentor]).to be_a(Legate::Tools::Base::HttpClient::QuietInstrumentor)
      end
    ...
    context 'with invalid base URL' do
      it 'raises Legate::ToolError for invalid URL' do
        # Now expects the error because rescue was removed from dummy initializer
        expect { DummyHttpToolWithClient.new(base_url: 'bad url') }.to raise_error(Legate::ToolError, /Invalid base_url/)
      end
      it 'raises Legate::ToolError for unsupported scheme' do
        expect { DummyHttpToolWithClient.new(base_url: 'ftp://x.com') }.to raise_error(Legate::ToolError, /Scheme must be/)
      end
    end
    context 'when Excon initialization fails' do
      ...
      it 'raises Legate::ToolError' do
        expect {
          DummyHttpToolWithClient.new(base_url: base_url)
        }.to raise_error(Legate::ToolError,
  ...
  describe 'request helpers' do
    ...
    describe '#http_get' do
      it 'requires setup_http_client first' do
        ...
        expect { tool_instance.http_get(path) }.to raise_error(Legate::ToolError, /HTTP client not initialized/)
      end
      ...
      it 'raises Legate::ToolError for invalid path chars' do
        ...
        }.to raise_error(Legate::ToolError, /Invalid URL or path provided: bad\0path - bad URI/)
      end
    ...
    describe '#http_post' do
      it 'requires setup_http_client first' do
        tool_instance = DummyHttpToolWithClient.new(base_url: base_url)
        tool_instance.instance_variable_set(:@http_client, nil)
        expect { tool_instance.http_post(path) }.to raise_error(Legate::ToolError, /HTTP client not initialized/)
      end
      it 'makes a request with correct method and relative path' do
      ...
      it 'raises error if Content-Type not JSON for Hash body' do
        ...
        }.to raise_error(Legate::ToolError, /Excon error.*InvalidStub.*Request body should be a string/i)
      end
      it 'raises Legate::ToolError if JSON encoding fails' do
        ...
        }.to raise_error(Legate::ToolError, /Failed to encode request body as JSON: json gen error/)
      end
    end
  ...
  describe 'error handling within make_request' do
    ...
    context 'when Excon raises TimeoutError' do
      ...
      it 'raises Legate::ToolTimeoutError' do
        expect { tool.http_get(path) }.to raise_error(Legate::ToolTimeoutError, /Timeout/)
      end
    end
    context 'when Excon raises SocketError' do
      ...
      it 'raises Legate::ToolError' do # Wrapped as generic error
        # Expect generic wrapping since we raised StandardError
        expect { tool.http_get(path) }.to raise_error(Legate::ToolError, /Unexpected error.*StandardError.*sock err/)
      end
    end
    context 'when Excon raises CertificateError' do
      ...
      it 'raises Legate::ToolError' do # Wrapped as generic error
        # Expect generic wrapping since we raised StandardError
        expect { tool.http_get(path) }.to raise_error(Legate::ToolError, /Unexpected error.*StandardError.*cert err/)
      end
    end
    context 'when request returns 4xx/5xx status code' do
      ...
      it 'raises Legate::ToolHttpError with response and cause' do
        expect { tool.http_get(path) }.to raise_error(Legate::ToolHttpError, /HTTP Error: Received status 404/) do |e|
      ...
      context 'with 503 status' do
        ...
        it 'raises Legate::ToolHttpError' do
          expect { tool.http_get(path) }.to raise_error(Legate::ToolHttpError, /HTTP Error: Received status 503/)
        end
      end
    end
    context 'when Excon raises other errors' do
      before { stub_request(method, error_url).to_raise(Excon::Error.new('generic')) }
      it 'raises Legate::ToolError' do # No cause check
        expect { tool.http_get(path) }.to raise_error(Legate::ToolError, /Excon error.*generic/)
      end
    end
    context 'when unexpected non-Excon error occurs' do
      ...
      it 'raises Legate::ToolError' do
        expect { tool.http_get(path) }.to raise_error(Legate::ToolError, /Unexpected error.*internal URI error/)
      end
    end
  end
end
