# File: spec/legate/session_service/in_memory_spec.rb
...
RSpec.describe Legate::SessionService::InMemory do
  ...
  describe '#initialize' do
    ...
    it 'logs initialization' do
      ...
      allow(Legate.logger).to receive(:info).with('InMemorySessionService initialized.')
  ...
  describe '#create_session' do
    ...
    it 'creates a new session with the correct attributes' do
      ...
      expect(session).to be_a(Legate::Session)
    ...
    it 'logs session creation' do
      expect(Legate.logger).to receive(:info).with(/Created session: .+ for app:#{app_name}, user:#{user_id}/).ordered
      service.create_session(app_name: app_name, user_id: user_id)
    end

    it 'honors an explicit session_id (used by the webhook listener to create a session run_task can find)' do
      session = service.create_session(app_name: app_name, user_id: user_id, session_id: 'fixed-webhook-id')
      expect(session.id).to eq('fixed-webhook-id')
      expect(service.get_session(session_id: 'fixed-webhook-id')).to eq(session)
    end
  end
  ...
  describe '#get_session' do
    ...
    it 'logs session retrieval success' do
      expect(Legate.logger).to receive(:debug).with("Retrieved session: #{session.id}")
      service.get_session(session_id: session.id)
    end

    it 'logs session not found' do
      expect(Legate.logger).to receive(:warn).with('Session not found: nonexistent_id')
  ...
  describe '#save_session (deprecated)' do
    ...
    let(:non_existent_session) do
      Legate::Session.new(app_name: 'other', user_id: 'other', session_service: service)
    end

    it 'returns true for an existing session and updates timestamp' do
      original_updated_at = session.updated_at
      sleep(0.01) # Ensure time progresses
      expect(Legate.logger).to receive(:warn).with('InMemorySessionService#save_session called (likely unnecessary). Use append_event.')
    ...
    it 'returns false for a non-existent session' do
      expect(Legate.logger).to receive(:error).with("Attempted to save non-existent session: #{non_existent_session.id}")
  ...
  describe '#append_event' do
    ...
    let(:event) { Legate::Event.new(role: :user, content: 'test event content', state_delta: { 'count' => 1 }) }
  ...
  describe '#delete_session' do
    ...
    it 'deletes an existing session' do
      expect(service.sessions.key?(session.id)).to be true
      expect(Legate.logger).to receive(:info).with("Deleted session: #{session.id}")
      expect(service.delete_session(session_id: session.id)).to be true
      expect(service.sessions.key?(session.id)).to be false
    end

    it 'returns false if the session ID does not exist' do
      expect(Legate.logger).to receive(:warn).with('Attempted to delete non-existent session: nonexistent_id')
  ...
  describe '#list_sessions' do
    ...
    it 'logs the count of listed sessions' do
      expect(Legate.logger).to receive(:debug).with('Listing 3 sessions.')
      service.list_sessions # No filters
      expect(Legate.logger).to receive(:debug).with('Listing 1 sessions.')
