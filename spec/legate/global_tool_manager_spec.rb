# File: spec/legate/global_tool_manager_spec.rb
# frozen_string_literal: true

require 'spec_helper'
require 'legate/global_tool_manager'
require 'legate/tool' # Dependency for tool classes

# --- Mock Tool Classes for Testing ---
class GtmMockTool < Legate::Tool
  self.explicit_tool_name = :gtm_mock
  tool_description 'GTM Mock Tool Desc'
  parameter :p1, required: true
  def perform_execution(params, _context); { status: :success, result: "Mock p1: #{params[:p1]}" }; end
end

class GtmMockToolNoMeta < Legate::Tool; end

class GtmMockToolOther < Legate::Tool
  self.explicit_tool_name = :gtm_other
  tool_description 'Other GTM Tool'
  def perform_execution(_params, _context); { status: :success, result: 'Other' }; end
end

class GtmMockToolWithError < Legate::Tool
  ...
  def perform_execution(_params, _context); { status: :success, result: 'Should not reach' }; end
end
# --- End Mock Tool Classes ---

RSpec.describe Legate::GlobalToolManager do
  let(:logger_spy) { spy('Logger') }

  before do
    # Ensure a clean slate before each test
    described_class.reset!
    # Stub the Legate logger
  ...
  describe '.register_tool' do
    ...
    it 'warns and overwrites when registering a tool with the same name' do
      ...
      class GtmMockToolDuplicate < Legate::Tool
        self.explicit_tool_name = :gtm_mock
        tool_description 'Anon Desc'
        def perform_execution(_params, _context); { status: :success, result: 'Duplicate' }; end
      end
    ...
    it 'warns and skips registration if object is not an Legate::Tool subclass' do
    ...
    it 'uses klass.to_s in warning for anonymous non-tool classes' do
      ...
      expect(logger_spy).to receive(:warn).with(/Attempted to register non-tool class: #{anon_class}/)
  ...
  describe '.list_all_tools' do
    ...
    it 'handles tools with missing descriptions or parameters gracefully' do
      described_class.reset!
      class GtmMinimalTool < Legate::Tool
