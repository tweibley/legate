# File: spec/legate/event_spec.rb
...
RSpec.describe Legate::Event do
  let(:valid_role) { :user }
  ...
  describe '#initialize' do
    ...
    context 'when role is :tool_request or :tool_result' do
      it 'warns if tool_name is nil' do
        expect(Legate.logger).to receive(:warn).with(/event created without a valid :tool_name symbol/)
      ...
      it 'does not warn if tool_name is a valid Symbol' do
        expect(Legate.logger).not_to receive(:warn).with(/tool_name/)
    ...
    it 'warns and sets state_delta to nil if it is not a Hash' do
      expect(Legate.logger).to receive(:warn).with(/state_delta must be a Hash or nil, received Array/)
    ...
    it 'warns if content is of an unusual type' do
      ...
      expect(Legate.logger).to receive(:warn).with(/Content is of unusual type \(Object\): #{unusual_content.inspect}/)
    ...
    it 'does not warn for common content types (String, Hash, Array, Nil, Numeric, Boolean)' do
      expect(Legate.logger).not_to receive(:warn).with(/Content is of unusual type/)
  ...
  describe 'result accessors' do
    it 'exposes a successful result via #success?/#answer' do
      event = described_class.new(role: :agent, content: { status: :success, result: '42' })
      expect(event.success?).to be true
      expect(event.error?).to be false
      expect(event.answer).to eq('42')
      expect(event.error_message).to be_nil
    end

    it 'exposes an error via #error?/#error_message' do
      event = described_class.new(role: :agent, content: { status: :error, error_message: 'boom' })
    ...
    it 'returns non-Hash content as-is from #answer and is neither success nor error' do
      event = described_class.new(role: :user, content: 'hello')
      expect(event.success?).to be false
      expect(event.error?).to be false
      expect(event.answer).to eq('hello')
    end
  end
  ...
  describe '.from_h' do
    ...
    it 'deserializes a valid hash with string keys' do
      ...
      expect(event).to be_a(Legate::Event)
    ...
    it 'returns nil and logs error for invalid role' do
      ...
      expect(Legate.logger).to receive(:error).with(/Failed to parse timestamp or invalid role: Invalid role: invalid/)
    ...
    it 'returns nil and logs error for invalid timestamp format' do
      ...
      expect(Legate.logger).to receive(:error).with(/Failed to parse timestamp or invalid role: invalid xmlschema format: "not-a-time"/)
      expect(described_class.from_h(invalid_hash)).to be_nil
    end
    ...
    it 'returns nil and logs error for type errors (e.g., non-hash state_delta)' do
      ...
      expect(Legate.logger).to receive(:error).with(/Type error during deserialization.*Hash: #{invalid_hash.inspect}/)
      expect(described_class.from_h(invalid_hash)).to be_nil
    end
    ...
    it 'returns nil and logs error if role is missing' do
      ...
      expect(Legate.logger).to receive(:error).with(/Failed to parse timestamp or invalid role: Invalid role:/)
      expect(described_class.from_h(invalid_hash)).to be_nil
    end
  end
end
