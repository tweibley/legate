...
RSpec.describe Legate::Agents::ParallelAgent do
  ...
  describe '#run_task' do
    it 'executes sub-agents concurrently' do
      expect(sub_agent_1).to receive(:run_task).and_return(Legate::Event.new(role: :agent, content: { status: :success, result: 'Result 1' }))
      expect(sub_agent_2).to receive(:run_task).and_return(Legate::Event.new(role: :agent, content: { status: :success, result: 'Result 2' }))
    ...
    it 'handles agent returning error status' do
      allow(sub_agent_1).to receive(:run_task).and_return(Legate::Event.new(role: :agent, content: { status: :success, result: 'Result 1' }))
      allow(sub_agent_2).to receive(:run_task).and_return(Legate::Event.new(role: :agent, content: { status: :error, error_message: 'Logic Error' }))
