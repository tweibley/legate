...
# File: spec/legate/agent_spec.rb
require 'spec_helper'
...
require 'legate/tools/calculator' # Example tool
require 'legate/tools/echo'       # Example tool
require 'legate/tools/base_async_job_tool' # For job_id tests
require 'legate/tools/check_job_status_tool' # For job_id tests
require 'legate/errors'
require 'legate/mcp/client' # For MCP tests
require 'legate/mcp/tool_wrapper' # For MCP tests
require 'legate/global_tool_manager' # Required for mocking
require 'legate/global_definition_registry' # Required for mocking
require 'legate/global_definition_registry' # For mocking registry

# --- Mock Tools ---
class MockTool < Legate::Tool
...
class MockAnotherTool < Legate::Tool
...
class MockToolNeedsContext < Legate::Tool
...
class MockToolNoName < Legate::Tool
...
class MockToolWithPending < Legate::Tool
...
class MockToolWithError < Legate::Tool
  tool_description 'A tool that always errors.'
  self.explicit_tool_name = :error_tool
  parameter :fail_message, type: :string

  def perform_execution(params, _context)
    raise Legate::ToolError.new("Tool failed as requested: #{params[:fail_message]}")
  end
end

class MockToolWithArgError < Legate::Tool
  ...
  def perform_execution(params, _context)
    raise Legate::ToolArgumentError, 'Invalid number provided' unless params[:number] > 0

    { status: :success, result: params[:number] }
  end
end

class MockToolInvalidResult < Legate::Tool
...
RSpec.describe Legate::Agent do
  ...
  let(:planner_double) { instance_double(Legate::Planner, plan: []) }
  let(:session_service_double) {
    instance_double(Legate::SessionService::InMemory, get_session: session_double, append_event: true)
  }
  let(:fake_history) { [] }
  let(:session_double) do
    ...
    instance_double(Legate::Session, id: session_id, user_id: 'user1', app_name: 'app1').tap do |double|
  ...
  let(:mock_config) {
    instance_double(Legate::Configuration, session_service: session_service_double)
  }
  let(:mock_tool_manager) {
    class_double(Legate::GlobalToolManager, find_class: nil, registered_tool_names: [], reset!: true,
  ...
  def create_agent(**options)
    ...
    def_tool_names = options.delete(:tool_names_array) || tool_classes.map { |tc|
      begin
    ...
    ephemeral_definition = Legate::AgentDefinition.new
    ...
    def_tool_names.each do |tn|
      ...
      tool_class_to_find = case tn
                           ...
                           when :echo then Legate::Tools::Echo
                           when :calculator then Legate::Tools::Calculator
                           when :check_job_status then Legate::Tools::CheckJobStatusTool
                           # Find from the `tool_classes` let variable if it's a custom tool for a specific test context
                           else tool_classes.find { |tc_let|
                                  begin
                                    tc_let.tool_metadata[:name].to_sym
                                  rescue StandardError
                                    nil
                                  end == tn
                                }
  ...
  def create_agent_from_definition(def_double = mock_definition, **overrides)
    allow(def_double).to receive(:is_a?).with(Legate::AgentDefinition).and_return(true)
    ...
    Legate::Agent.new(
      ...
      planner_override: planner_svc
      # Any remaining overrides are ignored by Legate::Agent.new unless they are valid params for it
    )
  end

  before do
    allow(Legate).to receive(:logger).and_return(logger_double)
    allow(Legate).to receive(:config).and_return(mock_config)
    ...
    allow(mock_tool_manager).to receive(:find_class).with(:echo).and_return(Legate::Tools::Echo)
    allow(mock_tool_manager).to receive(:find_class).with(:calculator).and_return(Legate::Tools::Calculator)
    allow(mock_tool_manager).to receive(:find_class).with(:pending_tool).and_return(MockToolWithPending)
    allow(mock_tool_manager).to receive(:find_class).with(:check_job_status).and_return(Legate::Tools::CheckJobStatusTool)
  ...
  describe '#initialize with definition:' do
    ...
    it 'uses definition model_name if set, otherwise agent default' do
      ...
      expect(agent2.model_name).to eq(Legate::Agent::DEFAULT_MODEL)
    end
    ...
    it 'uses provided session_service if passed' do
      custom_service = instance_double(Legate::SessionService::Base)
    ...
    it 'warns if a tool class from definition is not found globally' do
      ...
      expect(logger_double).to have_received(:warn).with(/no registered tool for missing_tool.*Available tools:/m)
    end

    it 'raises ArgumentError if definition is not an Legate::AgentDefinition' do
      expect {
        Legate::Agent.new(definition: {}, session_service: session_service_double)
      }.to raise_error(ArgumentError, /must be initialized with an Legate::AgentDefinition object/)
    end

    it 'raises ArgumentError if definition object is missing required methods' do
      incomplete_definition = instance_double(Legate::AgentDefinition)
      allow(incomplete_definition).to receive(:is_a?).with(Legate::AgentDefinition).and_return(true)
      ...
      expect {
        Legate::Agent.new(definition: incomplete_definition, session_service: session_service_double)
      }.to raise_error(ArgumentError, /Provided definition object does not appear to be a valid Legate::AgentDefinition/)
    end

    it 'uses planner_override if provided' do
      custom_planner = instance_double(Legate::Planner)
    ...
    context 'with sub-agent instantiation' do
      ...
      before do
        ...
        # Stub the class method directly on Legate::GlobalDefinitionRegistry
        allow(Legate::GlobalDefinitionRegistry).to receive(:find).with(child_agent_name).and_return(child_definition)
        allow(Legate::GlobalDefinitionRegistry).to receive(:find).with(another_child_agent_name).and_return(another_child_definition)
        allow(Legate::GlobalDefinitionRegistry).to receive(:register) # Allow register to be called
      end

      context 'when sub_agents parameter is provided (programmatic override)' do
        let(:programmatic_child_instance) { Legate::Agent.new(definition: another_child_definition, session_service: session_service_double) }
        let(:parent_agent) do
          Legate::Agent.new(
        ...
        it 'assigns parent session service to programmatic sub-agent if sub-agent lacks one' do
          child_without_service_def = Legate::AgentDefinition.new.define { |d|
            d.name :child_no_svc
            d.instruction 'hi'
          }
          programmatic_child_no_service = Legate::Agent.new(definition: child_without_service_def)
          ...
          parent_with_child_no_service = Legate::Agent.new(
        ...
        it 'warns if programmatic sub-agent has a different session service' do
          different_session_service = instance_double(Legate::SessionService::InMemory, get_session: nil, append_event: nil)
          child_with_diff_service_def = Legate::AgentDefinition.new.define { |d|
            d.name :child_diff_svc
            d.instruction 'hi'
          }
          programmatic_child_diff_service = Legate::Agent.new(definition: child_with_diff_service_def, session_service: different_session_service)
        ...
        context 'single parent rule enforcement for programmatic sub-agents' do
          let!(:rogue_parent_definition) {
            Legate::AgentDefinition.new.define { |d|
              d.name :rogue_parent
              d.instruction 'rogue'
            }
          }
          let!(:rogue_parent) { Legate::Agent.new(definition: rogue_parent_definition, session_service: session_service_double) }
          let!(:child_with_rogue_parent_def) {
            Legate::AgentDefinition.new.define { |d|
              d.name :child_with_parent
              d.instruction 'already parented'
            }
          }
          let!(:child_with_rogue_parent) do
            # Programmatically create child and assign rogue_parent
            child = Legate::Agent.new(definition: child_with_rogue_parent_def, session_service: session_service_double)
          ...
          it 'logs an error and does not adopt a sub-agent that already has a different parent' do
            expect(logger_double).to receive(:error).with(/sub-agent 'child_with_parent' already has a different parent: 'rogue_parent'\. Skipping/)

            parent_trying_to_adopt = Legate::Agent.new(
          ...
          it 'handles a sub-agent appearing multiple times in programmatic list (idempotent parenting to self)' do
            ...
            child_instance_for_parenting = Legate::Agent.new(definition: child_definition, session_service: session_service_double)

            parent_with_duplicate_sub = Legate::Agent.new(
              definition: parent_definition_declaring_subs,
              session_service: session_service_double,
      ...
      context 'when sub_agents parameter is NOT provided (declarative instantiation)' do
        ...
        it 'logs an error and skips if a declared sub-agent definition is not found in registry' do
          allow(Legate::GlobalDefinitionRegistry).to receive(:find).with(child_agent_name).and_return(nil) # Simulate missing definition
          expect(logger_double).to receive(:error).with(/Could not find definition for sub-agent 'child_one'/)
          parent_with_missing_sub_def = Legate::Agent.new(definition: parent_definition_declaring_subs, session_service: session_service_double)
      ...
      context 'when definition has no sub_agent_names and no sub_agents parameter' do
        ...
        let(:parent_agent) { Legate::Agent.new(definition: parent_definition_no_subs, session_service: session_service_double) }
      ...
      context 'when definition is loaded from a store hash (simulating persistence)' do
        ...
        let(:loaded_parent_a_definition_obj) do
          Legate::AgentDefinition.from_hash(parent_a_definition_hash_from_store)
        end

        before do
          # Ensure ChildB's full definition object is in the GlobalDefinitionRegistry
          # This is what Legate::Agent#initialize will use to instantiate ChildB
          # Stub the class method directly on Legate::GlobalDefinitionRegistry
          allow(Legate::GlobalDefinitionRegistry).to receive(:find).with(child_b_name).and_return(child_b_definition_obj)
  ...
  # after Legate::Agent#initialize was refactored to require a definition object for initialization.
  ...
  describe 'Tool Management' do
    ...
    describe '#add_tool' do
      let(:new_tool_class) { Legate::Tools::Calculator }
      ...
      it 'logs error and does not add tool if name cannot be determined' do
        klass_no_name = Class.new(Legate::Tool) do
    ...
    describe '#tools' do
      it 'returns an array of tool instances based on its definition' do
        ...
        # CheckJobStatusTool is always auto-added
        agent_with_tools = create_agent(tool_names_array: %i[mock_tool another_tool])
        tool_instances = agent_with_tools.tools
        expect(tool_instances).to be_an(Array)
        expected_classes = [MockTool, MockAnotherTool, Legate::Tools::CheckJobStatusTool]
      ...
      it 'returns only auto-registered tools if definition has no tools' do
        ...
        expect(agent_no_def_tools.tools).to match_array([an_instance_of(Legate::Tools::CheckJobStatusTool)])
      end
      ...
      it 'skips tool instance creation and warns if name cannot be retrieved' do
        ...
        test_tool_class = Class.new(Legate::Tool) do
        ...
        registry = Legate::ToolRegistry.new
    ...
    describe '#available_tools_metadata' do
      it 'returns metadata list from the tool registry' do
        ...
        expect(tool_names_in_meta).to include(:check_job_status)
      end

      it 'returns only auto-registered tools if definition has no tools' do
        agent_no_def_tools = create_agent(tool_names_array: [])
        metadata = agent_no_def_tools.available_tools_metadata
        expect(metadata.map { |m| m[:name] }).to match_array([:check_job_status])
      end
    end
  ...
  describe '#run_task' do
    ...
    subject(:agent) {
      create_agent(
        ...
        session_service: Legate::SessionService::InMemory.new, # Use a real service for these tests
    ...
    before do
      ...
      expect(svc).to be_an_instance_of(Legate::SessionService::InMemory)
    ...
    context 'pre-execution checks' do
      it 'returns error event if agent is not running', :skip_agent_start do
        ...
        expect(result_event.content[:error_message]).to match(/is not running.*agent\.start.*agent\.ask/m)
      end
    ...
    context 'unexpected runtime error during planning' do
      before { allow(planner_double).to receive(:plan).and_raise(StandardError.new('planner exploded')) }
      ...
      it 'returns an error event' do
        ...
        expect(result.content[:error_message]).to include('planner exploded')
      end

      it 'records the failure event in the session history' do
        agent.run_task(session_id: session_id, user_input: user_input, session_service: real_session_service)
        error_events = session_double.events.select do |e|
          e.role == :agent && e.content.is_a?(Hash) && e.content[:status] == :error
        end
    ...
    context 'multi-step execution with job_id injection' do
      ...
      before do
        ...
        check_tool_instance = Legate::Tools::CheckJobStatusTool.new
    ...
    context 'delegation interception' do
      ...
      it 'maps agent_transfer_to_ tool to delegate_task' do
        ...
        mock_agent_tool = instance_double(Legate::Tools::AgentTool)
    ...
    context 'when plan is empty' do
      ...
      context 'with fallback_mode :error (default)' do
        ...
        it 'returns an error event' do
          ...
          # The error now surfaces as a clean result event (readable via Event#error?/#error_message).
          expect(final_event.error?).to be true
          expect(final_event.error_message).to match(/cannot fulfill this request/)
        end
      end
    end
    ...
    context 'with output_key state management' do
      ...
      let(:agent_with_output_key) do
        ...
        definition_with_key = Legate::AgentDefinition.new.define do |d|
      ...
      it 'does not call session_service.set_state if output_key is not defined' do
        definition_no_key = Legate::AgentDefinition.new.define do |d|
        ...
        allow(mock_tool_manager).to receive(:find_class).with(:mock_tool).and_return(MockTool)
        agent_no_output_key = Legate::Agent.new(definition: definition_no_key, session_service: real_session_service, planner_override: planner_double)
      ...
      it 'handles errors during set_state and logs them' do
        ...
        final_event = agent_with_output_key.run_task(session_id: session_id, user_input: user_input, session_service: real_session_service)
        expect(final_event).to be_an(Legate::Event)
      end
    end
  end # End Run Task
end
