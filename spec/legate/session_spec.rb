# File: spec/legate/session_spec.rb
...
RSpec.describe Legate::Session do
  let(:app_name) { 'test_app' }
  let(:user_id) { 'test_user' }
  let(:session_id) { SecureRandom.uuid }
  let(:mock_session_service) { instance_double(Legate::SessionService::Base) }
  let(:initial_state) { { key1: 'value1', key2: 123 } }
  let(:event1_data) { { role: :user, content: 'Hello' } }
  let(:event2_data) { { role: :agent, content: 'Hi there' } }
  let(:event1) { Legate::Event.from_h(event1_data) }
  let(:event2) { Legate::Event.from_h(event2_data) }
  ...
  describe '#add_event' do
    let(:new_event) { Legate::Event.new(role: :user, content: 'Another message') }
    let(:event_with_delta) {
      Legate::Event.new(role: :tool_result, tool_name: :test_tool, content: {}, state_delta: { new_key: 'new_value' })
    }
    ...
    it 'does not call update_state if state_delta is empty' do
      empty_delta_event = Legate::Event.new(role: :user, content: '', state_delta: {})
      expect(session).not_to receive(:update_state)
  ...
  describe 'State Management (get/set/update/delete/clear)' do
    ...
    context 'with prefixes (session_service interaction)' do
      ...
      # Identity-qualified namespaces the session passes to the service
      # (app_name='test_app', user_id='test_user', session_id=<uuid>).
      let(:user_ns) { 'user:test_app:test_user' }
      let(:app_ns) { 'app:test_app' }
      let(:temp_ns) { "temp:#{session_id}" }

      it '#get_state calls session_service.load_scoped_state for prefixed keys' do
        expect(mock_session_service).to receive(:load_scoped_state).with(user_ns, 'pref').and_return('user_value')
        expect(mock_session_service).to receive(:load_scoped_state).with(app_ns, 'setting').and_return(true)
      ...
      it '#set_state calls session_service.save_scoped_state for prefixed keys' do
        expect(mock_session_service).to receive(:save_scoped_state).with(user_ns, 'pref', 'new_user_val')
        expect(mock_session_service).to receive(:save_scoped_state).with(temp_ns, 'data', { complex: true })
      ...
      it '#update_state calls session_service.save_scoped_state for prefixed keys' do
        expect(mock_session_service).to receive(:save_scoped_state).with(user_ns, 'pref', 'val1')
        expect(mock_session_service).to receive(:save_scoped_state).with(app_ns, 'setting', 123)
      ...
      it '#delete_state calls session_service.clear_scoped_state for prefixed keys' do
        expect(mock_session_service).to receive(:clear_scoped_state).with(user_ns, 'pref')
        expect(mock_session_service).to receive(:clear_scoped_state).with(app_ns, 'setting')
        session.delete_state(user_prefixed_key)
        session.delete_state(app_prefixed_key)
      end

      it '#clear_state! calls session_service.clear_scoped_state for each owner namespace with wildcard' do
        expect(mock_session_service).to receive(:clear_scoped_state).with(user_ns, '*')
        expect(mock_session_service).to receive(:clear_scoped_state).with(app_ns, '*')
        expect(mock_session_service).to receive(:clear_scoped_state).with(temp_ns, '*')
        session.clear_state!
    ...
    context 'state validation' do
      it '#set_state raises SerializationError for non-serializable value' do
        ...
        }.to raise_error(Legate::SerializationError, /must be JSON-serializable/)
        expect { session.set_state(:bad_key, Object.new) }.to raise_error(Legate::SerializationError)
        expect { session.set_state(:bad_nested, { key: Time.now }) }.to raise_error(Legate::SerializationError)
      end
      ...
      it '#set_state raises InvalidPrefixError for invalid prefixes' do
        ...
        }.to raise_error(Legate::InvalidPrefixError, /Invalid state key prefix: invalid/)
      end
      ...
      it '#update_state raises SerializationError if any value is invalid' do
        expect { session.update_state({ good: 1, bad: Time.now }) }.to raise_error(Legate::SerializationError)
      end

      it '#update_state raises InvalidPrefixError if any key has invalid prefix' do
        expect { session.update_state({ 'user:good' => 1, 'bad:prefix' => 2 }) }.to raise_error(Legate::InvalidPrefixError)
      end

      it '#delete_state raises InvalidPrefixError for invalid prefixes' do
        expect { session.delete_state('invalid:key') }.to raise_error(Legate::InvalidPrefixError)
      end

      it '#get_state raises InvalidPrefixError for invalid prefixes (symmetric with set_state)' do
        # Previously get_state silently mis-routed any colon key (e.g. a URL) to
        # scoped state instead of validating like the writers do.
        expect { session.get_state('http://example.com') }.to raise_error(Legate::InvalidPrefixError)
      end

      it '#set_state rejects non-finite floats (NaN/Infinity) up front' do
        expect { session.set_state(:bad, Float::NAN) }.to raise_error(Legate::SerializationError)
        expect { session.set_state(:bad, Float::INFINITY) }.to raise_error(Legate::SerializationError)
        expect { session.set_state(:ok, 1.5) }.not_to raise_error
      end
    end
  end
  ...
  describe 'Private Methods' do
    ...
    describe '#validate_prefix!' do
      ...
      it 'does nothing for valid prefixes' do
        Legate::Session::VALID_PREFIXES.each do |prefix|
          expect { session.send(:validate_prefix!, prefix) }.not_to raise_error
        end
      end

      it 'raises InvalidPrefixError for invalid prefix' do
        expect { session.send(:validate_prefix!, 'invalid') }.to raise_error(Legate::InvalidPrefixError)
        expect { session.send(:validate_prefix!, 'users') }.to raise_error(Legate::InvalidPrefixError)
      end
    end
    ...
    describe '#validate_serializable!' do
      ...
      it 'raises SerializationError for non-serializable objects' do
        expect { session.send(:validate_serializable!, Time.now) }.to raise_error(Legate::SerializationError)
        expect { session.send(:validate_serializable!, Object.new) }.to raise_error(Legate::SerializationError)
        expect { session.send(:validate_serializable!, Set.new) }.to raise_error(Legate::SerializationError)
      end

      it 'raises SerializationError for nested non-serializable objects' do
        expect { session.send(:validate_serializable!, { a: 1, b: Time.now }) }.to raise_error(Legate::SerializationError)
        expect { session.send(:validate_serializable!, [1, 2, Object.new]) }.to raise_error(Legate::SerializationError)
      end
    end
  end
  ...
  describe 'Serialization (#to_h, .from_h)' do
    ...
    let(:session_for_serialization) do
      s = described_class.new(
        ...
        user_id: user_id,
        initial_state: { k: 'v' },
        events: [Legate::Event.new(role: :user, content: 'hi')] # Create event to get its ID
      )
    ...
    it '.from_h deserializes a valid hash correctly' do
      ...
      expect(deserialized_session).to be_a(Legate::Session)
    ...
    it '.from_h handles missing optional fields' do
      ...
      expect(deserialized).to be_a(Legate::Session)
