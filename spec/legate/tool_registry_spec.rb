...
# --- Mock Tool Classes ---
class RegistryMockTool < Legate::Tool
...
class RegistryMockToolNoMeta < Legate::Tool; end

class RegistryErrorTool < Legate::Tool
...
RSpec.describe Legate::ToolRegistry do
  subject(:registry) { described_class.new }
  let(:logger_spy) { spy('Logger') }

  before do
    # Stub the Legate logger for registry instance
    # Note: ToolRegistry uses Legate.logger, so we stub the main one
  ...
  describe '#register' do
    ...
    it 'warns and overwrites when registering a tool with the same name' do
      ...
      class RegistryMockToolV2 < Legate::Tool; end # A different class
    ...
    it 'returns false and logs error if class is not an Legate::Tool subclass' do
