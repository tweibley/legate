# File: spec/legate/web/webhook_listener_error_handling_spec.rb
...
RSpec.describe Legate::Web::WebhookListener do
  ...
  before(:each) do
    ...
    # Setup registry to find agent definition
    ...
    # Stub session service methods
    allow(session_service).to receive(:get_session).and_return(nil)
    allow(session_service).to receive(:create_session).and_return(instance_double(Legate::Session, id: 'test-session-id'))

    # Stub Concurrent::Promises.future to prevent actual thread spawning
  ...
  describe 'Error Handling Cases' do
    ...
    context 'with global validator' do
      ...
      it 'accepts requests that pass global validation' do
        ...
        # Mock the rest of the flow for this test
