...
require 'spec_helper'
require 'legate/auth/excon_middleware'
require 'legate/auth/middleware_factory'
require 'legate/auth/schemes/api_key'
require 'legate/auth/schemes/http_bearer'
require 'legate/auth/token_store'
require 'excon'

RSpec.describe Legate::Auth::ExconMiddleware do
  ...
  let(:api_key_credential) { Legate::Auth::Credential.new(auth_type: :api_key, api_key: 'test-api-key', location: 'header', name: 'X-API-Key') }
  ...
  let(:mock_token_manager) do
    Class.new do
      ...
      def get_token(scheme, credential)
        @tokens[[scheme, credential]] ||= Legate::Auth::ExchangedCredential.new(
  ...
  describe 'authentication flow' do
    ...
    let(:mock_stack) do
      ...
      allow(mock).to receive(:request_call) do |datum|
        ...
        datum[:authenticated] = true if datum[:request] && datum[:request][:headers] && datum[:request][:headers]['X-API-Key'] == 'test-api-key'
        datum
      end
      ...
      allow(mock).to receive(:response_call) do |datum|
        ...
        datum[:response] = if datum[:authenticated]
    ...
    context 'with authentication failures' do
      let(:scheme_with_failures) do
        Class.new(Legate::Auth::Scheme) do
      ...
      it 'respects max_retries limit' do
        ...
        max_retry_middleware = described_class.new(
          mock_stack,
          scheme: Class.new(Legate::Auth::Scheme) do
            def scheme_type
              :custom
            end
    ...
    context 'with rate limiting and server errors' do
      ...
      let(:rate_limit_scheme) do
        Class.new(Legate::Auth::Scheme) do
          ...
          def initialize(status_codes = %w[429 200])
      ...
      it 'retries on rate limiting (429) responses' do
        rate_limit_middleware = described_class.new(
          nil, # Pass nil for the stack parameter
          scheme: rate_limit_scheme.new(%w[429 200]),
          credential: api_key_credential,
          auto_retry: true,
          max_retries: 1,
      ...
      it 'retries on server errors (5xx) by default' do
        rate_limit_middleware = described_class.new(
          nil, # Pass nil for the stack parameter
          scheme: rate_limit_scheme.new(%w[500 200]),
          credential: api_key_credential,
          auto_retry: true,
          max_retries: 1,
          backoff_factor: 0.1 # Keep test fast
      ...
      it 'retries on custom status codes when specified' do
        custom_middleware = described_class.new(
          nil, # Pass nil for the stack parameter
          scheme: rate_limit_scheme.new(%w[418 200]), # 418 I'm a teapot
          credential: api_key_credential,
          auto_retry: true,
          max_retries: 1,
      ...
      it 'does not retry non-idempotent methods by default' do
        ...
        Excon.stubs.clear
        Excon.stub({}) do |params|
          raise Excon::Error::InternalServerError.new('Simulated 500 error for non-idempotent method') if params[:method].to_s.upcase == 'POST'
    ...
    context 'with token manager integration' do
      let(:token_manager_class) do
        Class.new do
          ...
          def get_token(scheme, credential)
            ...
            # Invoke callbacks
            @callbacks[:token_refreshed].each { |cb| cb.call(scheme, credential, token) } if @callbacks[:token_refreshed]
          ...
          def invalidate_token(scheme, credential)
            ...
            # Invoke callbacks
            return unless @callbacks[:token_invalidated]
      ...
      it 'uses token manager to get tokens' do
        ...
        custom_api_key_scheme = Class.new(Legate::Auth::Scheme) do
          def scheme_type
            :api_key
          end
          ...
          def apply_to_request(request, credential)
            ...
            request[:headers]['X-API-Key'] = if credential.respond_to?(:access_token)
  ...
  describe 'should_retry? logic' do
    ...
    it 'returns true for explicit retry_on status codes' do
      expect(middleware.send(:should_retry?,
                             { method: 'GET' },
                             { status: 418 })).to be true

      expect(middleware.send(:should_retry?,
                             { method: 'GET' },
                             { status: 429 })).to be true
    end

    it 'returns true for authentication errors' do
      allow(Legate::Auth::ToolIntegration).to receive(:authentication_error?).and_return(true)

      expect(middleware.send(:should_retry?,
                             { method: 'GET' },
                             { status: 403 })).to be true
    end

    it 'returns true for server errors (5xx)' do
      expect(middleware.send(:should_retry?,
                             { method: 'GET' },
                             { status: 500 })).to be true

      expect(middleware.send(:should_retry?,
                             { method: 'GET' },
                             { status: 503 })).to be true
    end

    it 'returns true when Retry-After header is present' do
      expect(middleware.send(:should_retry?,
                             { method: 'GET' },
                             { status: 200, headers: { 'Retry-After' => '60' } })).to be true
    end
  ...
  describe 'middleware factory' do
    it 'creates middleware instances for different schemes' do
      ...
      enhanced_middleware = Legate::Auth::MiddlewareFactory.create_api_key(
        api_key: 'test-api-key',
        location: :header,
        name: 'X-API-Key',
        auto_retry: true,
