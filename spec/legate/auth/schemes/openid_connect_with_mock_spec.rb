...
RSpec.describe Legate::Auth::Schemes::OpenIDConnect do
  let(:mock_provider) { Legate::Test::Support::MockAuthProviders::MockOpenIDConnectProvider.new }
  ...
  before do
    ...
    allow_any_instance_of(Legate::Auth::Schemes::OpenIDConnect).to receive(:fetch_discovery_document).and_return({
  ...
  describe 'with mock provider' do
    ...
    let(:scheme) do
      ...
      test_class = Class.new(described_class) do
        ...
        def get_userinfo(_access_token)
    ...
    describe '#authorization_url' do
      ...
      it 'generates a valid authorization URL with OIDC parameters' do
        ...
        expect(auth_url).to include("nonce=#{nonce}") if scheme.is_a?(Class) && scheme.instance_methods.include?(:build_authorization_uri)
      end
    end
    ...
    describe '#exchange_authorization_code' do
      ...
      it 'successfully exchanges code for tokens including id_token' do
        ...
        allow_any_instance_of(Legate::Auth::Schemes::OAuth2).to receive(:exchange_token) do |instance, config, credential|
        ...
        # Use exchange_token which is the correct method in Legate::Auth::Schemes::OAuth2
        ...
        it 'handles error response' do
          ...
          error_scheme = Class.new(described_class) do
            def get_userinfo(access_token)
              raise Legate::Auth::Errors::AuthenticationError, 'Invalid token' if access_token == 'invalid_token'
