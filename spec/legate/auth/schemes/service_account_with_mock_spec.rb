...
RSpec.describe Legate::Auth::Schemes::ServiceAccount do
  let(:mock_provider) { Legate::Test::Support::MockAuthProviders::MockServiceAccountProvider.new }
  ...
  before do
    ...
    allow_any_instance_of(Legate::Auth::Schemes::ServiceAccount).to receive(:validate!).and_return(true)

    # For the basic tests, use a fixed token but with all required fields
    allow_any_instance_of(Legate::Auth::Schemes::ServiceAccount).to receive(:exchange_token) do |instance, cred|
    ...
    allow_any_instance_of(Legate::Auth::Schemes::ServiceAccount).to receive(:create_signed_jwt).and_return('test.signed.jwt')
  end
