# File: spec/legate/auth/schemes/api_key_spec.rb
...
RSpec.describe Legate::Auth::Schemes::ApiKey do
  ...
  describe '#apply_to_request' do
    ...
    context 'with header location' do
      ...
      it 'handles the Excon middleware stack format correctly' do
        ...
        test_scheme = Legate::Auth::Schemes::ApiKey.new
    ...
    it 'raises an error if the credential is missing the API key' do
      ...
      }.to raise_error(Legate::Auth::Error, /API key not found in credential/)
    end

    it 'works with environment variable resolution' do
      # Set up environment variable
      ENV['TEST_API_KEY'] = 'env-api-key'
      credential_with_env = Legate::Auth::Credential.new(
        auth_type: :api_key,
