...
RSpec.describe Legate::Auth::Schemes::OpenIDConnect do
  ...
  before(:each) do
    ...
    allow_any_instance_of(Legate::Auth::Schemes::OpenIDConnect).to receive(:verify_id_token) do |_scheme, id_token, nonce, client_id|
      {
  ...
  describe '#initialize' do
    context 'with explicit endpoints' do
      ...
      it 'raises an error without authorization_url' do
        ...
        }.to raise_error(Legate::Auth::SchemeValidationError, /Authorization URL is required/)
        ENV.delete('FORCE_VALIDATE')
      end
      ...
      it 'raises an error without token_url' do
        ...
        }.to raise_error(Legate::Auth::SchemeValidationError, /Token URL is required/)
        ENV.delete('FORCE_VALIDATE')
      end
    end
  ...
  describe '#exchange_token' do
    ...
    context 'with a valid ID token' do
      it 'creates an exchanged credential with the token and ID token' do
        ...
        allow_any_instance_of(Legate::Auth::Schemes::OAuth2).to receive(:exchange_token)
        ...
        allow_any_instance_of(Legate::Auth::Schemes::OpenIDConnect).to receive(:verify_id_token)
