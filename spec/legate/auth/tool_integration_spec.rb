# File: spec/legate/auth/tool_integration_spec.rb
# frozen_string_literal: true

require 'spec_helper'
require 'legate/auth/scheme'
require 'legate/auth/error'
require 'legate/auth/tool_integration'
...
RSpec.describe Legate::Auth::ToolIntegration do
  describe '.apply_authentication' do
    let(:request) { { url: 'https://api.example.com/data' } }
    let(:credential) { Legate::Auth::Credential.new(auth_type: :api_key, api_key: 'test-api-key') }
    let(:scheme) { Legate::Auth::Schemes::ApiKey.new }
    ...
    context 'with a token store' do
      ...
      let(:exchanged_credential) { instance_double('Legate::Auth::ExchangedCredential', expired?: false) }
      ...
      it 'refreshes expired tokens if supported' do
        expired_token = instance_double('Legate::Auth::ExchangedCredential', expired?: true)
        refreshed_token = instance_double('Legate::Auth::ExchangedCredential', expired?: false)
  ...
  describe '.generate_cache_key' do
    it 'generates a unique key for API key schemes' do
      ...
      credential = Legate::Auth::Credential.new(auth_type: :api_key, api_key: 'test-api-key')
    ...
    it 'generates a unique key for HTTP Bearer schemes' do
      ...
      credential = Legate::Auth::Credential.new(auth_type: :http_bearer, bearer_token: 'test-token')
    ...
    it 'generates different keys for different schemes' do
      ...
      api_key_credential = Legate::Auth::Credential.new(auth_type: :api_key, api_key: 'test-api-key')
      bearer_credential = Legate::Auth::Credential.new(auth_type: :http_bearer, bearer_token: 'test-token')
