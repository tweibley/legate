# File: spec/legate/auth/token_store_spec.rb
...
RSpec.describe Legate::Auth::TokenStore do
  let(:session_service) { instance_double('Legate::SessionService::Base') }
  ...
  describe '#store' do
    ...
    it 'handles exceptions' do
      ...
      allow(Legate).to receive_message_chain(:logger, :error)
  ...
  describe '#get' do
    ...
    it 'clears and returns nil for expired tokens' do
      ...
      allow(Legate::Auth::ExchangedCredential).to receive(:from_h).and_return(expired_token)
      allow(token_store).to receive(:clear)
      allow(Legate).to receive_message_chain(:logger, :debug)
