...
RSpec.describe Legate::Auth::Coordinator do
  ...
  let(:test_coordinator_class) do
    Class.new(Legate::Auth::Coordinator) do
      attr_reader :auth_steps, :auth_results

      def initialize(scheme:, credential:, session_service:, token_store: nil, timeout: Legate::Auth::Coordinator::DEFAULT_TIMEOUT)
        super
        @auth_steps = []
        @auth_results = []
      end
  ...
  describe '#start' do
    it 'initializes the fiber and returns the first auth request' do
      ...
      expect(coordinator.status).to eq(Legate::Auth::Coordinator::Status::PENDING)
  ...
  describe '#resume' do
    it 'resumes the authentication flow with a response' do
      ...
      expect(coordinator.status).to eq(Legate::Auth::Coordinator::Status::PENDING)
      expect(coordinator.complete?).to be false
      expect(coordinator.auth_steps).to eq(%i[step1 step2])
      ...
      expect(final_result).to be_a(Legate::Auth::ExchangedCredential)
      ...
      expect(coordinator.status).to eq(Legate::Auth::Coordinator::Status::COMPLETED)
    ...
    it 'handles errors during authentication' do
      ...
      error_coordinator_class = Class.new(Legate::Auth::Coordinator) do
        protected

        def authenticate
          response = Fiber.yield({ step: 1, data: 'step1-data' })
          raise Legate::Auth::Error, "Authentication failed: #{response[:error]}"
        end
      end
      ...
      # Verify coordinator state
      expect(error_coordinator.status).to eq(Legate::Auth::Coordinator::Status::FAILED)
      expect(error_coordinator.complete?).to be true
      expect(error_coordinator.success?).to be false
      expect(error_coordinator.error).to be_a(Legate::Auth::Error)
    ...
    it 'handles timeouts' do
      ...
      # Verify coordinator state
      expect(quick_timeout_coordinator.status).to eq(Legate::Auth::Coordinator::Status::TIMEOUT)
      expect(quick_timeout_coordinator.complete?).to be true
      expect(quick_timeout_coordinator.success?).to be false
      expect(quick_timeout_coordinator.error).to be_a(Legate::Auth::Error)
  ...
  describe '#cancel' do
    it 'cancels an in-progress authentication flow' do
      ...
      # Verify coordinator state
      expect(coordinator.status).to eq(Legate::Auth::Coordinator::Status::CANCELLED)
      expect(coordinator.complete?).to be true
      expect(coordinator.success?).to be false
      expect(coordinator.error).to be_a(Legate::Auth::Error)
