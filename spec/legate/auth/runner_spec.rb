...
RSpec.describe Legate::Auth::Runner do
  let(:session_service) { Legate::SessionService::InMemory.new }
  let(:token_store) { Legate::Auth::TokenStore.new(session_service) }
  let(:token_manager) { Legate::Auth::TokenManager.new(token_store) }
  let(:runner) { Legate::Auth::Runner.new(session_service: session_service, token_store: token_store, token_manager: token_manager) }
    ...
    context 'with authentication' do
      ...
      context 'without a cached token' do
        it 'handles authentication requests' do
          ...
          coordinator = instance_double(Legate::Auth::Coordinators::OAuth2Coordinator)
  ...
  describe '#handle_auth_response' do
    ...
    it 'processes valid responses' do
      ...
      auth_coordinator = instance_double(Legate::Auth::Coordinators::OAuth2Coordinator)
