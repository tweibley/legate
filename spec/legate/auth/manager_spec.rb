...
RSpec.describe Legate::Auth::Manager do
  ...
  before(:each) do
    ...
    Legate::Auth::Manager.instance_variable_set(:@singleton__instance__, nil)
  end
  ...
  describe '#initialize' do
    it 'registers default built-in schemes' do
      expect(manager.get_scheme(:api_key)).to be_a(Legate::Auth::Schemes::ApiKey)
      expect(manager.get_scheme(:http_bearer)).to be_a(Legate::Auth::Schemes::HTTPBearer)
      expect(manager.get_scheme(:oauth2)).to be_a(Legate::Auth::Schemes::OAuth2)
      expect(manager.get_scheme(:oidc)).to be_a(Legate::Auth::Schemes::OIDC)
      expect(manager.get_scheme(:service_account)).to be_a(Legate::Auth::Schemes::ServiceAccount)
    end
  end
  ...
  describe '#register_scheme' do
    ...
    it 'raises an error if scheme is not an Legate::Auth::Scheme' do
      expect {
        manager.register_scheme('not a scheme')
      }.to raise_error(ArgumentError, /Scheme must be an Legate::Auth::Scheme/)
    end
  end
  ...
  describe '#register_credential' do
    ...
    it 'raises an error if credential is not an Legate::Auth::Credential' do
      expect {
        manager.register_credential('not a credential', :test)
      }.to raise_error(ArgumentError, /Credential must be an Legate::Auth::Credential/)
    end
  ...
  describe '#register_url_mapping' do
    ...
    it 'registers a URL pattern to a scheme and credential' do
      ...
      expect(scheme).to be_a(Legate::Auth::Schemes::ApiKey)
      expect(credential).to eq(api_key_credential)
    end

    it 'supports regex patterns' do
      manager.register_url_mapping(%r{api\.example\.com/v\d+}, :api_key, :my_api_key)
  ...
  describe '#find_scheme_and_credential' do
    ...
    it 'finds scheme and credential based on URL' do
      ...
      expect(result[0]).to be_a(Legate::Auth::Schemes::ApiKey)
    ...
    it 'finds scheme and credential based on scheme type' do
      ...
      expect(result[0]).to be_a(Legate::Auth::Schemes::OAuth2)
