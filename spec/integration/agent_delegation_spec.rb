...
RSpec.describe 'Agent Delegation Integration', :integration do
  ...
  let!(:echo_tool) { Legate::Tools::Echo }

  before(:each) do
    # Reset tool manager to ensure a clean slate for each test
    Legate::GlobalToolManager.reset!
    Legate::GlobalToolManager.register_tool(calculator_tool)
    Legate::GlobalToolManager.register_tool(echo_tool)

    # Register agent definitions globally
    allow(Legate::GlobalDefinitionRegistry).to receive(:find).with(:coordinator_agent).and_return(coordinator_definition)
    allow(Legate::GlobalDefinitionRegistry).to receive(:find).with(:calculator_agent).and_return(calculator_definition)
    allow(Legate::GlobalDefinitionRegistry).to receive(:find).with(:research_agent).and_return(research_definition)
    ...
    # Allow Legate::Agent.new with any arguments first (for the first coordinator and known agents)
  ...
  context 'with hierarchical agent structure' do
    let(:coordinator_agent) do
      agent = Legate::Agent.new(definition: coordinator_definition, session_service: mock_session_service)
      allow(agent).to receive(:running?).and_return(true)
      agent
    end

    let(:calculator_agent) do
      agent = Legate::Agent.new(definition: calculator_definition, session_service: mock_session_service)
      allow(agent).to receive(:running?).and_return(true)
      agent
    end

    let(:research_agent) do
      agent = Legate::Agent.new(definition: research_definition, session_service: mock_session_service)
      allow(agent).to receive(:running?).and_return(true)
      agent
    end
    ...
    describe 'delegation via agent hierarchy' do
      it 'can find and delegate to calculator agent' do
        ...
        allow(calculator_agent).to receive(:run_task).and_return(
          Legate::Event.new(role: :agent, content: { result: 42 })
        )
      ...
      it 'prevents delegation to agents not in delegation_targets' do
        ...
        new_agent_definition = Legate::AgentDefinition.new.define do |a|
          a.name :unauthorized_agent
          a.description 'Agent not in delegation targets'
          a.instruction 'You should not be called'
        end

        new_agent = Legate::Agent.new(definition: new_agent_definition, session_service: mock_session_service)
  ...
  context 'with direct delegation using transfer_to' do
    ...
    before do
      ...
      mock_calculator_agent = instance_double(Legate::Agent, name: :calculator_agent, running?: false)
    ...
    it 'maintains session continuity during delegation' do
      ...
      session_aware_calculator = instance_double(Legate::Agent, name: :calculator_agent, running?: false)
  ...
  context 'with execute_step processing agent_transfer_to tools' do
    ...
    before do
      ...
      calc_agent = instance_double(Legate::Agent, name: :calculator_agent, running?: false)
