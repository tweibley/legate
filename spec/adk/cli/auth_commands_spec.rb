# File: spec/adk/cli/auth_commands_spec.rb
# frozen_string_literal: true

require 'spec_helper'
require 'stringio'
require 'adk/cli/auth_commands'

RSpec.describe ADK::CLI::AuthCommandHelpers do
  let(:helper_class) do
    Class.new do
      include ADK::CLI::AuthCommandHelpers
    end
  end
  ...
  describe '#auth_manager' do
    it 'returns the auth manager singleton' do
      expect(helper.auth_manager).to eq(ADK::Auth::Manager.instance)
    end
  end
end

RSpec.describe ADK::CLI::AuthSchemeCommands do
  let(:auth_manager) { ADK::Auth::Manager.instance }
...
RSpec.describe ADK::CLI::AuthCredentialCommands do
  let(:auth_manager) { ADK::Auth::Manager.instance }
  let(:command) { described_class.new }
  ...
  describe '#list' do
    ...
    context 'when credentials exist' do
      before do
        cred = ADK::Auth::Credential.new(auth_type: :api_key, api_key: 'secret123secret')
...
RSpec.describe ADK::CLI::AuthMappingCommands do
  let(:auth_manager) { ADK::Auth::Manager.instance }
  let(:command) { described_class.new }
...
RSpec.describe ADK::CLI::AuthCommands do
  let(:auth_manager) { ADK::Auth::Manager.instance }
  let(:command) { described_class.new }
