...
require_relative 'support/dummy_tool' # Load dummy tool for tests
...
RSpec.describe ADK::Agent do
  ...
  describe '#initialize with definition:' do
    ...
    before do
      ...
      agent_def.define do |a|
        ...
        a.instruction instruction_from_def
        a.model_name model_from_def
        tool_names_from_def.each { |tn| a.use_tool tn }
      end
    ...
    it 'sets attributes from the definition object' do
      ...
      expect(agent_from_def.model_name).to eq(model_from_def)
    ...
    it 'registers tools specified in the definition' do
      ...
      agent_from_def 
      expect(tool_registry_instance).to have_received(:register).with(:echo, EchoTool)
      expect(tool_registry_instance).to have_received(:register).with(:calculator, CalcTool)
    end
  ...
  # --- ADK::Agent.define related tests need review/removal? ---
  # The define method now exists on AgentDefinition, not Agent.
