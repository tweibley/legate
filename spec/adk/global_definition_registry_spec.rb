# File: spec/adk/global_definition_registry_spec.rb
# frozen_string_literal: true

require 'spec_helper'
require 'adk/global_definition_registry'
require 'adk/agent' # AgentDefinition is in the agent.rb file

RSpec.describe ADK::GlobalDefinitionRegistry do
  # Create a real AgentDefinition for testing
  class TestAgentDefinition < ADK::AgentDefinition
    attr_reader :name
  ...
  describe '.register' do
    ...
    it 'returns false for non-AgentDefinition objects' do
      ...
      expect(ADK.logger).to receive(:error).with(/Invalid object passed to register/)
    ...
    it 'overwrites existing definitions with the same name' do
      ...
      expect(ADK.logger).to receive(:warn).with(/Overwriting existing definition for agent/)
  ...
  describe '.find' do
    ...
    it 'returns nil and logs a warning for non-symbol keys' do
      expect(ADK.logger).to receive(:warn).with(/Find called with non-symbol key/)
