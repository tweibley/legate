...
RSpec.describe ADK::Agent do
  ...
  before do
    ...
    allow(ADK::GlobalDefinitionRegistry).to receive(:find).with(:parent_agent).and_return(parent_definition)
    allow(ADK::GlobalDefinitionRegistry).to receive(:find).with(:child_agent).and_return(child_definition)
  ...
  describe '#transfer_to' do
    ...
    context 'when delegating to a valid target agent' do
      ...
      let(:child_result) do
        ADK::Event.new(role: :agent, content: { status: :success, message: "Task processed" })
      end
    ...
    context 'when target agent is not found in hierarchy' do
      before do
        ...
        other_definition = ADK::AgentDefinition.new.define do |a|
          a.name :other_agent
        ...
        allow(ADK::GlobalDefinitionRegistry).to receive(:find).with(:other_agent).and_return(other_definition)

        # Mock the Agent.new call to avoid real instantiation
        new_agent = instance_double(ADK::Agent, running?: false, name: :other_agent)
        allow(new_agent).to receive(:start)
        allow(new_agent).to receive(:run_task).and_return(
          ADK::Event.new(role: :agent, content: { message: "Task processed" })
        )
      ...
      it 'instantiates the agent from the definition' do
        expect(ADK::Agent).to receive(:new).with(
          hash_including(definition: an_instance_of(ADK::AgentDefinition))
        )
