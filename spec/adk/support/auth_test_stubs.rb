...
module ADK
  # Define ADK::Error for test stubs
      ...
      class OAuth2 < Scheme
        ...
        def exchange_authorization_code(code)
          ...
          if response.status == 200
            ...
            raise ADK::Auth::Errors::AuthenticationError, "OAuth2 token exchange failed: #{error_response[:error]}"
          end
        end
        ...
        def refresh_access_token(refresh_token)
          ...
          if response.status == 200
            ...
            error_response = JSON.parse(response.body, symbolize_names: true)
            raise ADK::Auth::Errors::AuthenticationError, "OAuth2 token refresh failed: #{error_response[:error]}"
          end
        end
        ...
        def client_credentials_flow
          ...
          if response.status == 200
            ...
            error_response = JSON.parse(response.body, symbolize_names: true)
            raise ADK::Auth::Errors::AuthenticationError, "OAuth2 client credentials flow failed: #{error_response[:error]}"
          end
        end
        ...
        def get_userinfo(access_token)
          ...
          begin
            ...
            if response.status == 200
              ...
              raise ADK::Auth::Errors::AuthenticationError, "Failed to get user information: #{response.body}"
            end
          rescue StandardError => e
            raise ADK::Auth::Errors::AuthenticationError, "Error fetching user information: #{e.message}"
          end
        end
      ...
      class ServiceAccount < Scheme
        ...
        def fetch_access_token
          ...
          if response.status == 200
            ...
            error_response = JSON.parse(response.body, symbolize_names: true)
            raise ADK::Auth::Errors::AuthenticationError, "Token fetch failed: #{error_response[:error]}"
          end
        end
      ...
      class FiberCoordinator
        ...
        def authenticate
          ...
          when :service_account, 'service_account'
            ...
            raise ADK::Auth::Errors::ConfigurationError, "Unsupported auth scheme type: #{scheme_type}"
          end
        end
        ...
        def refresh(credentials)
          ...
          when :service_account, 'service_account'
            ...
            raise ADK::Auth::Errors::ConfigurationError, "Unsupported auth scheme type for refresh: #{scheme_type}"
          end
        end
        
        def authenticate_oauth2
          # Generate state parameter for CSRF protection
          state = ADK::Auth::Security.generate_csrf_token
          ...
          response = Fiber.yield(ADK::Auth::Coordinators::FiberSuspension.new(auth_url, state))
          
          # Check for error in the response
          if response[:error]
            raise ADK::Auth::Errors::AuthenticationError, response[:error]
          end
          
          # Exchange authorization code for tokens
          begin
            token_data = @scheme.exchange_authorization_code(response[:code])
            @credentials = ADK::Auth::Credentials.new(**token_data)
            @credentials
          rescue => e
            raise ADK::Auth::Errors::AuthenticationError, "Failed to exchange authorization code: #{e.message}"
          end
        end
        ...
        def authenticate_service_account
          ...
          @credentials = ADK::Auth::Credentials.new(**token_response)
          @credentials
        end
        ...
        def refresh_oauth2(credentials)
          ...
          unless refresh_token
            raise ADK::Auth::Errors::AuthenticationError, "No refresh token available"
          end
          ...
          begin
            ...
            raise ADK::Auth::Errors::AuthenticationError, "Failed to refresh token: #{e.message}"
          end
        end
      end
    end
  ...
  # Session service for Redis
  module SessionService
    class Redis
      attr_reader :redis_client, :session_ttl, :enable_encryption
    
      def initialize(redis_client: nil, session_ttl: nil, enable_encryption: true)
        require 'redis'
        if redis_client
      ...
      def set_auth_data(session_id, data)
        # For testing, we'll mock the encryption by adding a prefix
        # In a real implementation, this would encrypt sensitive data
        encrypted_data = { encrypted: true }
        data.each do |k, v|
          encrypted_data[k] = v.is_a?(String) ? "ENC:#{v}" : v
        end
        
        # Store both the raw data and encrypted data for testing
        @raw_data = data
        @encrypted_data = encrypted_data
        
        # In real implementation, this would store the data in Redis
        @redis_client.set("auth:#{session_id}", encrypted_data.to_json) if @redis_client
      end
      
      def get_auth_data(session_id)
        # In a real implementation, this would decrypt sensitive data
        # For testing, we'll return the raw data directly
        @raw_data || {}
      end
    end
