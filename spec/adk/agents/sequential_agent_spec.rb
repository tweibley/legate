...
require 'spec_helper'
require 'adk/agents/sequential_agent'
require 'adk/session'
require 'adk/session_service/in_memory'
require 'adk/event'

RSpec.describe ADK::Agents::SequentialAgent do
  ...
  # Definitions
  let(:sub_agent_1_def) { ADK::AgentDefinition.new.define { |d| d.name :sub_agent_1; d.instruction 'I am 1'; d.output_key :result_1 } }
  let(:sub_agent_2_def) { ADK::AgentDefinition.new.define { |d| d.name :sub_agent_2; d.instruction 'I am 2'; d.output_key :result_2 } }
  ...
  describe '#run_task' do
    it 'executes sub-agents in order' do
      expect(sub_agent_1).to receive(:run_task).ordered.and_return(ADK::Event.new(role: :agent, content: { status: :success, result: 'Result 1' }))
      expect(sub_agent_2).to receive(:run_task).ordered.and_return(ADK::Event.new(role: :agent, content: { status: :success, result: 'Result 2' }))
    ...
    it 'passes previous result to next agent input' do
      ...
      expect(sub_agent_2).to receive(:run_task) do |args|
        ...
        ADK::Event.new(role: :agent, content: { status: :success, result: 'Result 2' })
      end
    ...
    it 'stops execution if a sub-agent fails' do
      allow(sub_agent_1).to receive(:run_task).and_return(ADK::Event.new(role: :agent, content: { status: :error, error_message: 'Boom' }))
