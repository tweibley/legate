...
RSpec.describe ADK::Agents::LoopAgent do
  ...
  # Real sub-agent
  let(:sub_agent_def) { ADK::AgentDefinition.new.define { |d| d.name :worker_agent; d.instruction 'Worker' } }
  let(:sub_agent) { ADK::Agent.new(definition: sub_agent_def, session_service: session_service) }
  ...
  describe '#run_task' do
    context 'with max iterations' do
      it 'runs sub-agent repeatedly until max iterations' do
        expect(sub_agent).to receive(:run_task).exactly(3).times.and_return(ADK::Event.new(role: :agent, content: { status: :success, result: 'Done' }))
    ...
    context 'with loop condition' do
      ...
      it 'stops when condition is met' do
        ...
        expect(sub_agent).to receive(:run_task).once.and_return(ADK::Event.new(role: :agent, content: { status: :success }))
    ...
    context 'with error in sub-agent' do
      it 'terminates loop on error' do
        allow(sub_agent).to receive(:run_task).and_return(ADK::Event.new(role: :agent, content: { status: :error, error_message: 'Failed' }))
