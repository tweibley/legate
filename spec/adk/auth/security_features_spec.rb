...
RSpec.describe 'Authentication Security Features' do
  describe 'Token Encryption in Session Service' do
    let(:session_service) { ADK::SessionService::Redis.new }
    ...
    it 'encrypts sensitive data before storage' do
      # Store the sensitive data
      session_service.set_auth_data(session_id, sensitive_data)
      
      # Get encrypted data directly from the session service
      encrypted_data = session_service.instance_variable_get(:@encrypted_data)
      
      # Verify encryption was applied to sensitive fields
      expect(encrypted_data[:access_token]).to include('ENC:')
      expect(encrypted_data[:refresh_token]).to include('ENC:')
      expect(encrypted_data[:id_token]).to include('ENC:')
      expect(encrypted_data[:client_secret]).to include('ENC:')
      
      # Ensure original values are not present in encrypted form
      expect(encrypted_data[:access_token]).not_to eq(sensitive_data[:access_token])
      expect(encrypted_data[:refresh_token]).not_to eq(sensitive_data[:refresh_token])
      expect(encrypted_data[:id_token]).not_to eq(sensitive_data[:id_token])
      expect(encrypted_data[:client_secret]).not_to eq(sensitive_data[:client_secret])
    end
    
    it 'decrypts data correctly when retrieved' do
  ...
  describe 'CSRF Protection' do
    ...
    it 'generates and verifies CSRF tokens' do
      # Generate token
      csrf_token = ADK::Auth::Security.generate_csrf_token
      
      # Verify it's the expected token (we stubbed SecureRandom)
      expect(csrf_token).to eq('test_csrf_token')
      
      # Verify with the token
      expect(ADK::Auth::Security.verify_csrf_token(csrf_token)).to be true
      
      # Verify with wrong token
      expect(ADK::Auth::Security.verify_csrf_token('wrong_token')).to be false
    end
