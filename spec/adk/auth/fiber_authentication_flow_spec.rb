...
RSpec.describe 'Fiber-based authentication flow' do
  ...
  describe 'Fiber-based OAuth2 coordinators' do
    ...
    let(:coordinator) { ADK::Auth::Coordinators::FiberCoordinator.new(oauth2_config) }
    ...
    it 'authenticates using the coordinator' do
      ...
      expect(result).to be_a(ADK::Auth::Coordinators::FiberSuspension)
      ...
      # Verify credentials
      expect(credentials).to be_a(ADK::Auth::Credentials)
    ...
    it 'handles token refresh' do
      ...
      expect(refreshed_credentials).to be_a(ADK::Auth::Credentials)
    ...
    it 'raises error when authentication fails' do
      ...
      bad_coordinator = ADK::Auth::Coordinators::FiberCoordinator.new(bad_config)
