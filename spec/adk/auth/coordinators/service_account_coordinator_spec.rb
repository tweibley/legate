...
require 'spec_helper'
require 'adk/auth/coordinators/service_account_coordinator'
require 'adk/auth/schemes/service_account'
require 'adk/auth/credential'
require 'adk/auth/token_store'
require 'securerandom'

RSpec.describe ADK::Auth::Coordinators::ServiceAccountCoordinator do
  # Add a generate_request_id method to ADK::Auth module if it doesn't exist
  before(:all) do
    unless ADK::Auth.respond_to?(:generate_request_id)
      ADK::Auth.define_singleton_method(:generate_request_id) { SecureRandom.uuid }
    end
  end
  ...
  let(:session_service) do
    instance_double('ADK::SessionService::Base')
  end
  
  let(:token_store) do
    instance_double(ADK::Auth::TokenStore)
  end
  ...
  before do
    ...
    allow(service_account_scheme).to receive(:is_a?).with(ADK::Auth::Schemes::ServiceAccount).and_return(true)
  ...
  describe '#initialize' do
    ...
    it 'raises an error with invalid scheme type' do
      invalid_scheme = instance_double('ADK::Auth::Scheme')
      allow(invalid_scheme).to receive(:is_a?).with(ADK::Auth::Schemes::ServiceAccount).and_return(false)
    ...
    it 'raises an error with invalid credential type' do
      invalid_credential = instance_double(ADK::Auth::Credential)
