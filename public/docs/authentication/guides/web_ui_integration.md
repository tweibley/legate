...
The ADK Web UI includes an authentication status indicator that shows the current authentication state for tools requiring authentication:
...
4. The provider redirects back to the ADK callback URL
...
<div class="auth-flow-visualization">
  ...
  <div class="flow-step" data-step="2">
    ...
    <div class="step-description">ADK yields for authentication</div>
  </div>
  <div class="flow-arrow">→</div>
  ...
  <div class="flow-step" data-step="4">
    ...
    <div class="step-description">ADK exchanges code for tokens</div>
  </div>
  <div class="flow-arrow">→</div>
  <div class="flow-step" data-step="5">
    <div class="step-number">5</div>
    <div class="step-description">ADK resumes with valid tokens</div>
  </div>
</div>
```

## Integration with ADK Core

### Web UI Authentication Coordinator

The ADK Web UI integrates with the ADK core authentication system through the `WebUIAuthenticationCoordinator` class:
  ...
  module Web
    class WebUIAuthenticationCoordinator
      ...
      def handle_auth_request(auth_config)
        ...
        @app.session[:oauth2_state] = state
        
        # Prepare authentication configuration for the Web UI
        client_config = {
          auth_request_id: auth_config.request_id,
          auth_type: auth_config.scheme.auth_type,
          authorization_url: auth_config.scheme.auth_url,
          client_id: auth_config.credential.client_id,
          redirect_uri: @app.url("/auth/callback"),
          scopes: auth_config.scheme.scopes,
          state: state
        }
        
        # Return configuration to be sent to the Web UI
        client_config
      end
      
      def handle_auth_response(params)
        # Create the authentication response
        auth_response = Adk::Auth::Config.new(
          request_id: params[:auth_request_id],
          auth_response_uri: params[:auth_response_uri],
          redirect_uri: params[:redirect_uri]
        )
        
        # Return the response to be resumed to the Fiber
        auth_response
      end
    end
  end
...
Here's a complete example of implementing OAuth2 authentication in the ADK Web UI:
...
post '/api/auth/request' do
  ...
  coordinator = Adk::Web::WebUIAuthenticationCoordinator.new(self)
...
post '/api/auth/response' do
  ...
  coordinator = Adk::Web::WebUIAuthenticationCoordinator.new(self)
  auth_response = coordinator.handle_auth_response(params)
  
  # Store the response in the session for the next Fiber.resume
