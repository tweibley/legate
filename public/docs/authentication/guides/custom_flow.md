...
While the ADK Ruby library provides built-in support for common authentication schemes, you may need to implement custom authentication flows for APIs with unique requirements. This guide explains how to create custom authentication flows using the ADK's authentication framework.
...
Custom authentication schemes should inherit from the `Adk::Auth::Scheme` base class:
...
```ruby
class CustomInteractiveScheme < Adk::Auth::Scheme
  ...
  def exchange_auth_code(credential, code, redirect_uri)
    ...
    if response.status != 200
      raise Adk::Auth::Error::TokenExchangeError, "Failed to exchange code: #{response.body}"
    end
  ...
  def refresh_tokens(credential, tokens)
    ...
    if response.status != 200
      raise Adk::Auth::Error::TokenRefreshError, "Failed to refresh token: #{response.body}"
    end
    ...
    module Coordinators
      class CustomInteractiveCoordinator
        ...
        def handle_auth_response(config)
          ...
          unless code
            raise Adk::Auth::Error::AuthenticationError, "Missing authorization code in response"
          end
...
custom_credential = Adk::Auth::Credential.new(
  client_id: ENV['CUSTOM_CLIENT_ID'],
  client_secret: ENV['CUSTOM_CLIENT_SECRET']
)

# Create a tool using the custom scheme
class CustomApiTool < Adk::Tool::FunctionTool
  def initialize
    super(
      name: 'custom_api_tool',
      description: 'A tool that uses custom authentication',
  ...
  def call(context, **params)
    ...
    unless tokens
      ...
      if auth_response
        ...
        coordinator = Adk::Auth::Coordinators::CustomInteractiveCoordinator.new(
          context.auth_scheme,
          context.auth_credential
        )
        
        tokens = coordinator.handle_auth_response(auth_response)
...
RSpec.describe 'Custom Authentication Integration' do
  ...
  it 'successfully completes the authentication flow' do
    ...
    runner.auth_response = Adk::Auth::Config.new(
...
def exchange_auth_code(credential, code, redirect_uri)
  ...
  when 401
    raise Adk::Auth::Error::CredentialError, "Invalid client credentials"
  else
    raise Adk::Auth::Error::TokenExchangeError, "Failed to exchange code: #{response.body}"
  end
end
...
def handle_error(error_data)
  ...
  case error
  when 'invalid_grant'
    raise Adk::Auth::Error::TokenExchangeError, "Invalid authorization code"
  when 'invalid_client'
    raise Adk::Auth::Error::CredentialError, "Invalid client credentials"
  when 'invalid_request'
    raise Adk::Auth::Error::TokenExchangeError, "Invalid request: #{error_data['error_description']}"
  else
    raise Adk::Auth::Error::TokenExchangeError, "Authentication error: #{error_data['error_description'] || error}"
  end
end
```
