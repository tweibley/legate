...
This section provides Mermaid sequence diagrams to visualize how different callbacks intercept and potentially alter the typical execution flow of an ADK Agent interacting with an LLM (via its Planner).
...
sequenceDiagram
    participant User
    participant AgentRunner as ADK Runner/Framework
...
2.  The `ADK Runner/Framework` calls the `before_agent_callback`.
