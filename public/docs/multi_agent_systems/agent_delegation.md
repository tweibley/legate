# Agent Delegation in ADK-Ruby

## Overview

Agent delegation is a powerful feature in ADK-Ruby that allows one agent to dynamically transfer control to another specialized agent during execution. This enables complex workflows where a coordinator agent can make decisions about which specialized agent should handle a particular task.
...
```ruby
coordinator_agent = ADK::Agent.define do |a|
...
math_agent = ADK::Agent.define do |a|
  a.name :math_agent
...
coordinator = ADK::Agent.define do |a|
  a.name :coordinator
...
coordinator_instance = ADK::Agent.new(definition: coordinator)
math_instance = ADK::Agent.new(definition: math_agent)
...
translator_agent = ADK::Agent.define do |a|
...
For detailed implementation examples, see the `examples/mas/delegation_example.rb` file in the ADK-Ruby codebase. 