# Sequential Agents in ADK-Ruby

Sequential agents are a powerful feature in ADK-Ruby that allow you to compose multiple specialized agents into a workflow where agents are executed in a predetermined sequence. This approach lets you break down complex tasks into smaller, more manageable components that work together.
...
The ADK provides two different implementations of a travel planner using sequential agents, demonstrating different approaches to agent orchestration:
...
```bash
bundle exec ruby examples/travel_planner_auto_sequential.rb
```
...
```bash
bundle exec ruby examples/travel_planner_sequential.rb
```
...
travel_planner_def = ADK::AgentDefinition.new.define do |a|
  ...
  a.agent_type :sequential  # This tells ADK to use SequentialAgent
...
2. Register all sub-agent definitions with `ADK::GlobalDefinitionRegistry`
3. Define your main sequential agent with:
   ```ruby
   sequential_agent = ADK::AgentDefinition.new.define do |a|
