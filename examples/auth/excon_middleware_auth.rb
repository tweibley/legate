...
# 
# This example demonstrates how to use the ADK::Auth::ExconMiddleware with various
# authentication schemes, including automatic token refresh, retry functionality, 
# and configurable backoff strategies.
#
# Usage:
#   ruby examples/auth/excon_middleware_auth.rb
...
puts "ADK::Auth Enhanced Excon Middleware Example"
puts "----------------------------------------"
...
begin
  # Create a connection using the convenience method
  connection = ADK::Auth.create_basic_auth_connection(
    'https://httpbin.org',
    username: 'testuser',
...
class RetryCounterScheme < ADK::Auth::Scheme
  def initialize
    @attempts = 0
    @max_failures = 2
  end
...
class RateLimitScheme < ADK::Auth::Scheme
  def initialize
    @attempts = 0
  ...
  def apply_to_request(request, credential)
    ...
    if @attempts <= @rate_limit_count
      ...
      request[:headers]['X-Rate-Limited'] = 'true'
    else
      # This will succeed
      request[:headers]['X-Rate-Limited'] = 'false'
    end
...
begin
  ...
  rate_limit_credential = ADK::Auth::Credential.new(auth_type: :custom)
  
  # Create a middleware that will handle rate limiting
  rate_limit_middleware = ADK::Auth::MiddlewareFactory.create(
