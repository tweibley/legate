...
# Example of using OpenID Connect (OIDC) authentication with ADK
#
...
#
# Usage:
#   ruby examples/auth/oidc_auth.rb [--with-server]
#   ruby examples/auth/oidc_auth.rb --client-id=YOUR_CLIENT_ID --client-secret=YOUR_CLIENT_SECRET --provider=google
#
...
OptionParser.new do |opts|
  opts.banner = "Usage: ruby examples/auth/oidc_auth.rb [options]"
...
def display_user_info(user_info)
  ...
  if user_info[:sub]
    puts "  Subject ID  : #{user_info[:sub]}"
  end
  
  # Display additional fields if available
  additional_fields = user_info.keys - [:name, :email, :picture, :locale, :sub, :error]
...
def make_authenticated_request(token, oidc_scheme)
  ...
  if token[:id_token]
    puts "  ID Token    : #{token[:id_token][0..9]}...#{token[:id_token][-4..-1]}"
  end
