...
class MockPlanner
  ...
  def plan(user_input)
    ...
    if @agent.definition.delegation_targets.include?(:calculator_agent) && 
        (user_input =~ /\d\s*[\+\-\*\/]\s*\d/ || user_input.downcase.include?('calculate') || user_input.downcase.include?('math'))
...
class CalculatorTool < ADK::Tool
  ...
  def call(params)
    begin
      result = eval(params[:expression].gsub(/[^0-9\+\-\*\/\(\)\.]/,''))
      { result: result }
    rescue => e
...
unless ADK::Agent.instance_methods.include?(:public_execute_step)
  ADK::Agent.class_eval do
    ...
    def public_execute_step(step, session, session_service)
      ...
      self.send(:execute_step, step, session, session_service)
    end
  end
end
